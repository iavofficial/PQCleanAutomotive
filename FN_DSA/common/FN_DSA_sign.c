...
/*====================================================================================================================*/
/** \file FN_DSA_sign.c
* \brief  description of FN_DSA_sign.c
*
* \details
*
*
...
#define FN_DSA_MAX_UINT32 0xFFFFFFFFu
/**********************************************************************************************************************/
/* TYPES                                                                                                              */
/**********************************************************************************************************************/
 ...
 * sigma = 1/isigma.
 * fn_dsa_Gaussian0Sampler() takes as parameter a pointer to a PRNG, and returns an integer sampled along a half-Gaussian
...
/* GLOBAL VARIABLES                                                                                                   */
/**********************************************************************************************************************/
static const uint32 fn_dsa_dist[] = {
...
/**********************************************************************************************************************/
static uint32 fn_dsa_FfldlTreeSize(uint32 logn);
static void fn_dsa_FfldlFftInner(fpr *const tree, fpr *const g0, fpr *const g1, uint32 logn, fpr *const tmp);
static void fn_dsa_FfldlBinaryNormalize(fpr *const tree, uint32 orig_logn, uint32 logn);
static void fn_dsa_SmallintsToFpr(fpr *const r, const sint8 *const t, uint32 logn);
static sint32 fn_dsa_BerExp(prng *const p, fpr x, fpr ccs);
static sint32 fn_dsa_SignSampler(void *const ctx, fpr mu, fpr isigma);
static sint32 fn_dsa_Gaussian0Sampler(prng *const p);
static void fn_dsa_FfSamplingFftDyntree(samplerZ const samp, void *const samp_ctx, fpr *const t0, fpr *const t1,
                                              fpr *const g00, fpr *const g01, fpr *const g11, uint32 orig_logn,
                                              uint32 logn, fpr *const tmp);
static void fn_dsa_FfSamplingFft(samplerZ const samp, void *const samp_ctx, fpr *const z0, fpr *const z1,
                                       const fpr *const tree, const fpr *const t0, const fpr *const t1, uint32 logn,
                                       fpr *const tmp);
static sint32 fn_dsa_DoSignDyn(samplerZ const samp, void *const samp_ctx, sint16 *const s2, const sint8 *const f,
...
static void fn_dsa_FfldlFftInner(fpr *const tree, fpr *const g0, fpr *const g1, uint32 logn, fpr *const tmp)
{
  ...
  if (FALSE == bStopFunc)
  {
    ...
    * d11 into tmp. */
    FN_DSA_Poly_LdlMvFFT(tmp, tree, g0, g1, g0, logn);

    /* Split d00 (currently in g0) and d11 (currently in tmp). We reuse g0 and g1 as temporary storage spaces:
    *   d00 splits into g1, g1+hn
    *   d11 splits into g0, g0+hn */
    FN_DSA_Poly_SplitFFT(g1, &g1[hn], g0, logn);
    FN_DSA_Poly_SplitFFT(g0, &g0[hn], tmp, logn);
    ...
    fn_dsa_FfldlFftInner(&tree[n], g1, &g1[hn], logn - 1u, tmp);
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfldlFftInner(&tree[n + fn_dsa_FfldlTreeSize(logn - 1u)], g0, &g0[hn], logn - 1u, tmp);
  }
} // end: fn_dsa_FfldlFftInner
...
static void fn_dsa_FfldlBinaryNormalize(fpr *const tree, uint32 orig_logn, uint32 logn)
{
  ...
  if (n == 1u)
  {
    ...
    tree[0] = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Sqrt(tree[0]), fpr_inv_sigma[orig_logn]);
  }
  else
  {
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfldlBinaryNormalize(&tree[n], orig_logn, logn - 1u);
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfldlBinaryNormalize(&tree[n + fn_dsa_FfldlTreeSize(logn - 1u)], orig_logn, logn - 1u);
  }
} // end: fn_dsa_FfldlBinaryNormalize
...
static void fn_dsa_SmallintsToFpr(fpr *const r, const sint8 *const t, uint32 logn)
{
  ...
  for (u = 0; u < n; u++)
  {
    r[u] = FN_DSA_Fpr_Of(t[u]);
  }
} // end: fn_dsa_SmallintsToFpr
...
static sint32 fn_dsa_BerExp(prng *const p, fpr x, fpr ccs)
{
   ...
   * FN_DSA_Fpr_Trunc(). */
  s = (sint32)FN_DSA_Fpr_Trunc(FN_DSA_Fpr_Mul(x, fpr_inv_log2));
  r = FN_DSA_Fpr_Sub(x, FN_DSA_Fpr_Mul(FN_DSA_Fpr_Of(s), fpr_log2));
   ...
   * which is approximatively equal to 2^(-32). In any case, if s >= 64, then fn_dsa_BerExp will be non-zero with probability
  ...
  /* Compute exp(-r); we know that 0 <= r < log(2) at this point, so we can use FN_DSA_Fpr_ExpmP63(), which
   ...
   * in that case). The bias is negligible since FN_DSA_Fpr_ExpmP63() only computes with 51 bits of precision
   * or so. */
  z = ((FN_DSA_Fpr_ExpmP63(r, ccs) << 1) - 1u) >> (uint32)s;
  ...
  {
    i -= 8;
    w = FN_DSA_Prng_GetU8(p) - (uint32)((uint64)(((uint64)(z >> (uint64)i)) & 0xFFu));
  } while ((0u == w) && (i > 0));
...
*        The value of sigma MUST lie between 1 and 2 (i.e. isigma lies between 0.5 and 1); in FN_DSA, sigma
...
static sint32 fn_dsa_SignSampler(void *const ctx, fpr mu, fpr isigma)
{
  ...
  s = (sint32)FN_DSA_Fpr_Floor(mu);
  r = FN_DSA_Fpr_Sub(mu, FN_DSA_Fpr_Of(s));

  /* dss = 1/(2*sigma^2) = 0.5*(isigma^2). */
  dss = FN_DSA_Fpr_Half(FN_DSA_Fpr_Sqr(isigma));

  /* ccs = sigma_min / sigma = sigma_min * isigma. */
  ccs = FN_DSA_Fpr_Mul(isigma, spc->sigma_min);

  /* We now need to sample on center r. */
  for (uint32 i = 0; i < FN_DSA_MAX_UINT32; i++)
  {
    /* Sample z for a Gaussian distribution. Then get a random bit b to turn the sampling into a bimodal
       ...
       */
    z0 = fn_dsa_Gaussian0Sampler(&spc->p);
    b  = (sint32)((uint32)(FN_DSA_Prng_GetU8(&spc->p) & 1u));
    ...
    x = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Sqr(FN_DSA_Fpr_Sub(FN_DSA_Fpr_Of(z), r)), dss);
    x = FN_DSA_Fpr_Sub(
        x, FN_DSA_Fpr_Mul(FN_DSA_Fpr_Of((sint64)((sint64)z0 * (sint64)z0)), fpr_inv_2sqrsigma0));

    if (0 < fn_dsa_BerExp(&spc->p, x, ccs))
    {
...
static sint32 fn_dsa_Gaussian0Sampler(prng *const p)
{
  ...
  lo = FN_DSA_Prng_GetU64(p);
  hi = FN_DSA_Prng_GetU8(p);
  ...
  for (u = 0; u < ((sizeof(fn_dsa_dist)) / sizeof(fn_dsa_dist[0])); u += 3u)
  {
    uint32 w0, w1, w2, cc;
                                              ...
                                              uint32 logn, fpr *const tmp)
{
  ...
  if (logn == 0u)
  {
    ...
    leaf  = g00[0];
    leaf  = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Sqrt(leaf), fpr_inv_sigma[orig_logn]);
    t0[0] = FN_DSA_Fpr_Of(samp(samp_ctx, t0[0], leaf));
    t1[0] = FN_DSA_Fpr_Of(samp(samp_ctx, t1[0], leaf));
  ...
  if (FALSE == bStopFunc)
  {
    ...
    FN_DSA_Poly_LdlFFT(g00, g01, g11, logn);
   ...
   * back into tmp + 2*n. */
    z1 = &tmp[n];
    FN_DSA_Poly_SplitFFT(z1, &z1[hn], t1, logn);
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfSamplingFftDyntree(samp, samp_ctx, z1, &z1[hn], g11, &g11[hn], &g01[hn], orig_logn, logn - 1u,
                                      &z1[n]);
    FN_DSA_Poly_MergeFFT(&tmp[(n << 1)], z1, &z1[hn], logn);
   ...
   * In the end, z1 is written over t1, and tb0 is in t0. */
    ...
    z0 = tmp;
    FN_DSA_Poly_SplitFFT(z0, &z0[hn], t0, logn);
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfSamplingFftDyntree(samp, samp_ctx, z0, &z0[hn], g00, &g00[hn], g01, orig_logn, logn - 1u, &z0[n]);
    FN_DSA_Poly_MergeFFT(t0, z0, &z0[hn], logn);
  }
} // end: fn_dsa_FfSamplingFftDyntree
                                       ...
                                       fpr *const tmp)
{
  ...
  if (logn == 2u)
  {
    ...
    b_re = t1[1];
    b_im = t1[3];
    c_re = FN_DSA_Fpr_Add(a_re, b_re);
    c_im = FN_DSA_Fpr_Add(a_im, b_im);
    w0   = FN_DSA_Fpr_Half(c_re);
    w1   = FN_DSA_Fpr_Half(c_im);
    c_re = FN_DSA_Fpr_Sub(a_re, b_re);
    c_im = FN_DSA_Fpr_Sub(a_im, b_im);
    w2   = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Add(c_re, c_im), fpr_invsqrt8);
    w3   = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Sub(c_im, c_re), fpr_invsqrt8);

    x0    = w2;
    x1    = w3;
    sigma = tree1[3];
    w2    = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    w3    = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));
    a_re  = FN_DSA_Fpr_Sub(x0, w2);
    a_im  = FN_DSA_Fpr_Sub(x1, w3);
    b_re  = tree1[0];
    b_im  = tree1[1];
    c_re  = FN_DSA_Fpr_Sub(FN_DSA_Fpr_Mul(a_re, b_re), FN_DSA_Fpr_Mul(a_im, b_im));
    c_im  = FN_DSA_Fpr_Add(FN_DSA_Fpr_Mul(a_re, b_im), FN_DSA_Fpr_Mul(a_im, b_re));
    x0    = FN_DSA_Fpr_Add(c_re, w0);
    x1    = FN_DSA_Fpr_Add(c_im, w1);
    sigma = tree1[2];
    w0    = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    w1    = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));

    a_re  = w0;
    a_im  = w1;
    b_re  = w2;
    b_im  = w3;
    c_re  = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Sub(b_re, b_im), fpr_invsqrt2);
    c_im  = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Add(b_re, b_im), fpr_invsqrt2);
    z1[0] = FN_DSA_Fpr_Add(a_re, c_re);
    w0    = FN_DSA_Fpr_Add(a_re, c_re);
    z1[2] = FN_DSA_Fpr_Add(a_im, c_im);
    w2    = FN_DSA_Fpr_Add(a_im, c_im);
    z1[1] = FN_DSA_Fpr_Sub(a_re, c_re);
    w1    = FN_DSA_Fpr_Sub(a_re, c_re);
    z1[3] = FN_DSA_Fpr_Sub(a_im, c_im);
    w3    = FN_DSA_Fpr_Sub(a_im, c_im);

    /* Compute tb0 = t0 + (t1 - z1) * L. Value tb0 ends up in w*. */
    w0 = FN_DSA_Fpr_Sub(t1[0], w0);
    w1 = FN_DSA_Fpr_Sub(t1[1], w1);
    w2 = FN_DSA_Fpr_Sub(t1[2], w2);
    w3 = FN_DSA_Fpr_Sub(t1[3], w3);

    a_re = w0;
    a_im = w2;
    b_re = tree[0];
    b_im = tree[2];
    w0   = FN_DSA_Fpr_Sub(FN_DSA_Fpr_Mul(a_re, b_re), FN_DSA_Fpr_Mul(a_im, b_im));
    w2   = FN_DSA_Fpr_Add(FN_DSA_Fpr_Mul(a_re, b_im), FN_DSA_Fpr_Mul(a_im, b_re));
    a_re = w1;
    a_im = w3;
    b_re = tree[1];
    b_im = tree[3];
    w1   = FN_DSA_Fpr_Sub(FN_DSA_Fpr_Mul(a_re, b_re), FN_DSA_Fpr_Mul(a_im, b_im));
    w3   = FN_DSA_Fpr_Add(FN_DSA_Fpr_Mul(a_re, b_im), FN_DSA_Fpr_Mul(a_im, b_re));

    w0 = FN_DSA_Fpr_Add(w0, t0[0]);
    w1 = FN_DSA_Fpr_Add(w1, t0[1]);
    w2 = FN_DSA_Fpr_Add(w2, t0[2]);
    w3 = FN_DSA_Fpr_Add(w3, t0[3]);
    ...
    sigma = tree0[3];
    w2    = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    y0    = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    w3    = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));
    y1    = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));
    a_re  = FN_DSA_Fpr_Sub(x0, y0);
    a_im  = FN_DSA_Fpr_Sub(x1, y1);
    b_re  = tree0[0];
    b_im  = tree0[1];
    c_re  = FN_DSA_Fpr_Sub(FN_DSA_Fpr_Mul(a_re, b_re), FN_DSA_Fpr_Mul(a_im, b_im));
    c_im  = FN_DSA_Fpr_Add(FN_DSA_Fpr_Mul(a_re, b_im), FN_DSA_Fpr_Mul(a_im, b_re));
    ...
    c_re  = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Sub(b_re, b_im), fpr_invsqrt2);
    c_im  = FN_DSA_Fpr_Mul(FN_DSA_Fpr_Add(b_re, b_im), fpr_invsqrt2);
    z0[0] = FN_DSA_Fpr_Add(a_re, c_re);
    z0[2] = FN_DSA_Fpr_Add(a_im, c_im);
    z0[1] = FN_DSA_Fpr_Sub(a_re, c_re);
    z0[3] = FN_DSA_Fpr_Sub(a_im, c_im);

    bStopFunc = TRUE;
  }

  /* Case logn == 1 is reachable only when using FN_DSA-2 (the smallest size for which FN_DSA is mathematically
  ...
  if (logn == 1u)
  {
    ...
    x1    = t1[1];
    sigma = tree[3];
    z1[0] = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    y0    = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    z1[1] = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));
    y1    = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));
    a_re  = FN_DSA_Fpr_Sub(x0, y0);
    a_im  = FN_DSA_Fpr_Sub(x1, y1);
    b_re  = tree[0];
    ...
    c_re  = FN_DSA_Fpr_Sub(FN_DSA_Fpr_Mul(a_re, b_re), FN_DSA_Fpr_Mul(a_im, b_im));
    c_im  = FN_DSA_Fpr_Add(FN_DSA_Fpr_Mul(a_re, b_im), FN_DSA_Fpr_Mul(a_im, b_re));
    x0    = FN_DSA_Fpr_Add(c_re, t0[0]);
    x1    = FN_DSA_Fpr_Add(c_im, t0[1]);
    sigma = tree[2];
    z0[0] = FN_DSA_Fpr_Of(samp(samp_ctx, x0, sigma));
    z0[1] = FN_DSA_Fpr_Of(samp(samp_ctx, x1, sigma));
  ...
  if (FALSE == bStopFunc)
  {
    ...
    hn    = n >> 1;
    tree0 = &tree[n];
    tree1 = &tree[n + fn_dsa_FfldlTreeSize(logn - 1u)];
   ...
   * We finally merge back into z1. */
    ...
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfSamplingFft(samp, samp_ctx, tmp, &tmp[hn], tree1, z1, &z1[hn], logn - 1u, &tmp[n]);
    ...
    /* Second recursive invocation. */
    FN_DSA_Poly_SplitFFT(z0, &z0[hn], tmp, logn);
    /* polyspace +2 CERT-C:MEM05-C [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    /* polyspace +1 MISRA2012:17.2 [Justified:]"Without in-depth knowledge, this violation cannot be resolved." */
    fn_dsa_FfSamplingFft(samp, samp_ctx, tmp, &tmp[hn], tree0, z0, &z0[hn], logn - 1u, &tmp[n]);
    FN_DSA_Poly_MergeFFT(z0, tmp, &tmp[hn], logn);
  }
} // end: fn_dsa_FfSamplingFft
                                     ...
                                     const uint16 *const hm, uint32 logn, fpr *const tmp)
{
  ...
  fn_dsa_SmallintsToFpr(b00, g, logn);
  fn_dsa_SmallintsToFpr(b11, F, logn);
  fn_dsa_SmallintsToFpr(b10, G, logn);
  FN_DSA_FFT(b01, logn);
  FN_DSA_FFT(b00, logn);
  FN_DSA_FFT(b11, logn);
  FN_DSA_FFT(b10, logn);
  FN_DSA_Poly_Neg(b01, logn);
  FN_DSA_Poly_Neg(b11, logn);
  ...
  for (u = 0; u < n; u++)
  {
    t0[u] = FN_DSA_Fpr_Of((sint64)hm[u]);
    /* This is implicit. 
     * t1[u] = fpr_zero */
  }
  ...
  fn_dsa_FfSamplingFftDyntree(samp, samp_ctx, t0, t1, g00, g01, g11, logn, logn, &t1[n]);
  ...
  for (u = 0; u < n; u++)
  {
    z = (sint32)hm[u] - (sint32)FN_DSA_Fpr_Rint(t0[u]);
  ...
  for (u = 0; u < n; u++)
  {
    s2tmp[u] = (sint16)-FN_DSA_Fpr_Rint(t1[u]);
  }
                           ...
                           uint32 logn, uint8 *const tmp)
{
  ...
  for (uint32 i = 0; i < FN_DSA_MAX_UINT32; i++)
  {
    ...
    FN_DSA_Prng_Init(&spc.p, rng);
    samp     = fn_dsa_SignSampler;
    samp_ctx = &spc;

    /* Do the actual signature. */
    if (0 < fn_dsa_DoSignDyn(samp, samp_ctx, sig, f, g, F, G, hm, logn, ftmp))
    {
      break;
    }
  }
