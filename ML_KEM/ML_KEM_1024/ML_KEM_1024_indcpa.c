...
/*====================================================================================================================*/
/** \file ML_KEM_1024_indcpa.c
* \brief  description of ML_KEM_1024_indcpa.c
*
* \details
*
*
...
/**********************************************************************************************************************/
static void ml_kem_1024_PackPk(uint8 r[ML_KEM_1024_INDCPA_PUBLICKEYBYTES], const polyvec1024 *const pk,
                                   const uint8 seed[ML_KEM_SYMBYTES]);
static void ml_kem_1024_UnpackPk(polyvec1024 *const pk, uint8 seed[ML_KEM_SYMBYTES],
                                     const uint8 packedpk[ML_KEM_1024_INDCPA_PUBLICKEYBYTES]);
static void ml_kem_1024_PackSk(uint8 r[ML_KEM_1024_INDCPA_SECRETKEYBYTES], const polyvec1024 *const sk);
static void ml_kem_1024_UnpackSk(polyvec1024 *const sk, const uint8 packedsk[ML_KEM_1024_INDCPA_SECRETKEYBYTES]);
static void ml_kem_1024_PackCiphertext(uint8 r[ML_KEM_1024_INDCPA_BYTES], const polyvec1024 *const b,
                                           const poly *const v);
static void ml_kem_1024_UnpackCiphertext(polyvec1024 *const b, poly *const v,
                                             const uint8 c[ML_KEM_1024_INDCPA_BYTES]);
static uint16 ml_kem_1024_RejUniform(sint16 *const r, uint16 len, const uint8 *const buf, uint16 buflen);
                                   ...
                                   const uint8 seed[ML_KEM_SYMBYTES])
{
  ...
  ML_KEM_1024_Polyvec_ToBytes(r, pk);

  for (i = 0; i < ML_KEM_SYMBYTES; i++)
  {
    r[i + ML_KEM_1024_POLYVECBYTES] = seed[i];
  }
} // end: ml_kem_1024_PackPk

/*====================================================================================================================*/
/**
* \brief De-serialize public key from a byte array
*        approximate inverse of ml_kem_1024_PackPk
*
                                     ...
                                     const uint8 packedpk[ML_KEM_1024_INDCPA_PUBLICKEYBYTES])
{
  ...
  ML_KEM_1024_Polyvec_FromBytes(pk, packedpk);

  for (i = 0; i < ML_KEM_SYMBYTES; i++)
  {
    seed[i] = packedpk[i + ML_KEM_1024_POLYVECBYTES];
  }
} // end: ml_kem_1024_UnpackPk
...
static void ml_kem_1024_PackSk(uint8 r[ML_KEM_1024_INDCPA_SECRETKEYBYTES], const polyvec1024 *const sk)
{
  ML_KEM_1024_Polyvec_ToBytes(r, sk);
} // end: ml_kem_1024_PackSk

/*====================================================================================================================*/
/**
* \brief De-serialize the secret key; inverse of ml_kem_1024_PackSk
*
...
static void ml_kem_1024_UnpackSk(polyvec1024 *const sk, const uint8 packedsk[ML_KEM_1024_INDCPA_SECRETKEYBYTES])
{
  ML_KEM_1024_Polyvec_FromBytes(sk, packedsk);
} // end: ml_kem_1024_UnpackSk
...
static void ml_kem_1024_PackCiphertext(uint8 r[ML_KEM_1024_INDCPA_BYTES], const polyvec1024 *const b,
                                           const poly *const v)
{
  ML_KEM_1024_Polyvec_Compress(r, b);
  ML_KEM_1024_Poly_Compress(&r[ML_KEM_1024_POLYVECCOMPRESSEDBYTES], v);
} // end: ml_kem_1024_PackCiphertext

/*====================================================================================================================*/
/**
* \brief De-serialize and decompress ciphertext from a byte array;
*        approximate inverse of ml_kem_1024_PackCiphertext
*
...
*/
static void ml_kem_1024_UnpackCiphertext(polyvec1024 *const b, poly *const v, const uint8 c[ML_KEM_1024_INDCPA_BYTES])
{
  ML_KEM_1024_Polyvec_Decompress(b, c);
  ML_KEM_1024_Poly_Decompress(v, &c[ML_KEM_1024_POLYVECCOMPRESSEDBYTES]);
} // end: ml_kem_1024_UnpackCiphertext
...
void ML_KEM_1024_Indcpa_GenMatrix(polyvec1024 *a, const uint8 seed[ML_KEM_SYMBYTES], uint8 transposed)
{
  ...
  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    for (j = 0; j < ML_KEM_1024_K; j++)
    {
      if (transposed > 0u)
      {
      ...
      ctr    = ml_kem_1024_RejUniform(a[i].vec[j].coeffs, ML_KEM_N, buf, buflen);

      while (ctr < ML_KEM_N)
      {
        FsmSw_Fips202_Shake128_SqueezeBlocks(buf, 1u, &state);
        buflen = XOF_BLOCKBYTES;
        ctr    = ctr + (ml_kem_1024_RejUniform(&(a[i].vec[j].coeffs[ctr]), ML_KEM_N - ctr, buf, buflen));
      }
    }
  }
} // end: ML_KEM_1024_Indcpa_GenMatrix
...
*
* \param[out] uint8 *pk : pointer to output public key (of length ML_KEM_1024_INDCPA_PUBLICKEYBYTES bytes)
* \param[out] uint8 *sk : pointer to output private key  (of length ML_KEM_1024_INDCPA_SECRETKEYBYTES bytes)
*/
                                    ...
                                    uint8 sk[ML_KEM_1024_INDCPA_SECRETKEYBYTES])
{
  ...
  polyvec1024 a[ML_KEM_1024_K], e, pkpv, skpv;

  (void)FsmSw_CommonLib_RandomBytes(buf, ML_KEM_SYMBYTES);
  // Domain separation byte
  buf[ML_KEM_SYMBYTES] = ML_KEM_1024_K;
  FsmSw_Fips202_Sha3_512(buf, buf, ML_KEM_SYMBYTES + 1);

  ML_KEM_1024_Indcpa_GenMatrix(a, publicseed, 0);

  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    ML_KEM_1024_Poly_GetNoiseEta1(&skpv.vec[i], noiseseed, nonce);
    nonce++;
  }

  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    ML_KEM_1024_Poly_GetNoiseEta1(&e.vec[i], noiseseed, nonce);
    nonce++;
  }

  ML_KEM_1024_Polyvec_Ntt(&skpv);
  ML_KEM_1024_Polyvec_Ntt(&e);

  // matrix-vector multiplication
  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    ML_KEM_1024_Polyvec_BasemulAccMontgomery(&pkpv.vec[i], &a[i], &skpv);
    ML_KEM_Poly_Tomont(&pkpv.vec[i]);
  }

  ML_KEM_1024_Polyvec_Add(&pkpv, &pkpv, &e);
  ML_KEM_1024_Polyvec_Reduce(&pkpv);

  ml_kem_1024_PackSk(sk, &skpv);
  ml_kem_1024_PackPk(pk, &pkpv, publicseed);
} // end: ML_KEM_1024_Indcpa_KeyPair
...
*        public-key encryption scheme underlying ML_KEM.
*
* \param[out] uint8           *c : pointer to output ciphertext (of length ML_KEM_1024_INDCPA_BYTES bytes)
* \param[in]  const uint8     *m : pointer to input message (of length ML_KEM_1024_INDCPA_MSGBYTES bytes)
* \param[in]  const uint8    *pk : pointer to input public key (of length ML_KEM_1024_INDCPA_PUBLICKEYBYTES)
* \param[in]  const uint8 *coins : pointer to input random coins used as seed
*                                  (of length ML_KEM_SYMBYTES) to deterministically
                                ...
                                const uint8 pk[ML_KEM_1024_INDCPA_PUBLICKEYBYTES], const uint8 coins[ML_KEM_SYMBYTES])
{
  ...
  polyvec1024 pkpv            = {{{{0}}}};
  polyvec1024 ep              = {{{{0}}}};
  polyvec1024 at[ML_KEM_1024_K] = {{{{{0}}}}};
  polyvec1024 b               = {{{{0}}}};
  poly v                      = {{0}};
  poly k                      = {{0}};
  poly epp                    = {{0}};

  ml_kem_1024_UnpackPk(&pkpv, seed, pk);
  ML_KEM_1024_Poly_FromMsg(&k, m);
  ML_KEM_1024_Indcpa_GenMatrix(at, seed, 1);

  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    ML_KEM_1024_Poly_GetNoiseEta1(&(sp.vec[i]), coins, nonce);
    nonce++;
  }

  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    ML_KEM_1024_Poly_GetNoiseEta2(&(ep.vec[i]), coins, nonce);
    nonce++;
  }

  ML_KEM_1024_Poly_GetNoiseEta2(&epp, coins, nonce);

  ML_KEM_1024_Polyvec_Ntt(&sp);

  // matrix-vector multiplication
  for (i = 0; i < ML_KEM_1024_K; i++)
  {
    ML_KEM_1024_Polyvec_BasemulAccMontgomery(&b.vec[i], &at[i], &sp);
  }

  ML_KEM_1024_Polyvec_BasemulAccMontgomery(&v, &pkpv, &sp);

  ML_KEM_1024_Polyvec_InvnttTomont(&b);
  ML_KEM_Poly_InvnttTomont(&v);

  ML_KEM_1024_Polyvec_Add(&b, &b, &ep);
  ML_KEM_Poly_Add(&v, &v, &epp);
  ML_KEM_Poly_Add(&v, &v, &k);
  ML_KEM_1024_Polyvec_Reduce(&b);
  ML_KEM_Poly_Reduce(&v);

  ml_kem_1024_PackCiphertext(c, &b, &v);
} // end: ML_KEM_1024_Indcpa_Enc
...
*        public-key encryption scheme underlying ML_KEM.
*
* \param[out] uint8        *m :  pointer to output decrypted message (of length ML_KEM_1024_INDCPA_MSGBYTES bytes)
* \param[in]  const uint8  *c :  pointer to input ciphertext (of length ML_KEM_1024_INDCPA_BYTES bytes)
* \param[in]  const uint8 *sk : pointer to input secret key (of length ML_KEM_1024_INDCPA_SECRETKEYBYTES bytes)
*/
                                ...
                                const uint8 sk[ML_KEM_1024_INDCPA_SECRETKEYBYTES])
{
  ...
  ml_kem_1024_UnpackCiphertext(&b, &v, c);
  ml_kem_1024_UnpackSk(&skpv, sk);

  ML_KEM_1024_Polyvec_Ntt(&b);
  ML_KEM_1024_Polyvec_BasemulAccMontgomery(&mp, &skpv, &b);
  ML_KEM_Poly_InvnttTomont(&mp);

  ML_KEM_Poly_Sub(&mp, &v, &mp);
  ML_KEM_Poly_Reduce(&mp);

  ML_KEM_1024_Poly_ToMsg(m, &mp);
} // end: ML_KEM_1024_Indcpa_Dec
