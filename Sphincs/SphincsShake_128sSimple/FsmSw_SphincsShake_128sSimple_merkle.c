 ...
 *
 */
void FsmSw_SphincsShake_128sSimple_Merkle_Sign(uint8 *sig, uint8 *root, const sphincs_shake_128s_ctx *ctx,
                                               const uint32 wots_addr[8], uint32 tree_addr[8], uint32 idx_leaf)
{
  uint8 *auth_path = &sig[FSMSW_SPHINCSSHAKE_128SSIMPLE_WOTS_BYTES];
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void FsmSw_SphincsShake_128sSimple_Merkle_GenRoot(uint8 *root, const sphincs_shake_128s_ctx *ctx)
{
