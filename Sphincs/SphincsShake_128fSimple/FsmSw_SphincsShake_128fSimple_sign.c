                                                         ...
                                                         uint32 mlen, const uint8 *const sk)
{
  ...
  for (i = 0; i < FSMSW_SPHINCSSHAKE_128FSIMPLE_D; i++)
  {
    ...
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << FSMSW_SPHINCSSHAKE_128FSIMPLE_TREE_HEIGHT)) - 1u));
                                                     ...
                                                     uint32 smlen, const uint8 *const pk)
{
  ...
  *mlen = smlen - FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES;
  ...
  if (0 != FsmSw_SphincsShake_128fSimple_Crypto_Sign_Verify(sm, FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES,
                                                            &sm[FSMSW_SPHINCSSHAKE_128FSIMPLE_BYTES], *mlen, pk))
  {
    FsmSw_CommonLib_MemSet(m, 0, smlen);
    *mlen  = 0;
    retVal = ERR_NOT_OK;
