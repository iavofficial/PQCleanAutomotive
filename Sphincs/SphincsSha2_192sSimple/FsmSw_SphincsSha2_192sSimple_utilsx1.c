...
void FsmSw_SphincsSha2_192sSimple_TreeHashX1(uint8 *root, uint8 *auth_path, const sphincs_sha2_192s_ctx *ctx,
                                             ...
                                             void (*gen_leaf)(uint8 *out /* Where to write the leaves */,
                                                              const sphincs_sha2_192s_ctx *ctx, uint32 idx, void *info),
                                             uint32 tree_addr[8], void *info)
{
  ...
  for (idx = 0; idx < 0xFFFFFFFFu; idx++)
  {
    ...
    for (h = 0; h < 0xFFFFFFFFu; h++)
    {
      ...
      uint8 *left = &stack[h * FSMSW_SPHINCSSHA2_192SSIMPLE_N];
