...
/*====================================================================================================================*/
/** \file ML_DSA_87_poly.c
* \brief  description of ML_DSA_87_poly.c
*
* \details
*
*
...
#define POLY_UNIFORM_GAMMA1_NBLOCKS ((POLYZ_PACKEDBYTES_ML_DSA_87 + STREAM256_BLOCKBYTES - 1u) / STREAM256_BLOCKBYTES)
...
/**********************************************************************************************************************/
static uint32 fsmsw_ml_dsa_87_RejEta(sint32 *const a, uint32 len, const uint8 *const buf, uint32 buflen);
static uint32 fsmsw_ml_dsa_87_RejUniform(sint32 *const a, uint32 len, const uint8 *const buf, uint32 buflen);
...
static uint32 fsmsw_ml_dsa_87_RejUniform(sint32 *const a, uint32 len, const uint8 *const buf, uint32 buflen)
{
  ...
  return ctr;
} // end: fsmsw_ml_dsa_87_RejUniform
/**********************************************************************************************************************/
...
*
* \param[in,out] poly_D5 *const a: pointer to input/output polynomial
*/
void ML_DSA_87_Poly_Reduce(poly_D5 *const a)
{
  uint16 i = 0;

  for (i = 0; i < N_ML_DSA; ++i)
  {
    a->coeffs[i] = ML_DSA_Reduce32(a->coeffs[i]);
  }
} // end: ML_DSA_87_Poly_Reduce
/*====================================================================================================================*/
/**
...
} // end: ML_DSA_87_Poly_CAddQ
/*====================================================================================================================*/
/**
* \brief Add polynomials. No modular reduction is performed.
*
* \param[out] poly_D5       *const c : pointer to output polynomial
* \param[in]  const poly_D5 *const a : pointer to first summand
* \param[in]  const poly_D5 *const b : pointer to second summand
*/
...
} // end: ML_DSA_87_Poly_Add
/*====================================================================================================================*/
/**
* \brief Subtract polynomials. No modular reduction is performed.
*
* \param[out] poly_D5       *const c : pointer to output polynomial
* \param[in]  const poly_D5 *const a : pointer to first input polynomial
* \param[in]  const poly_D5 *const b : pointer to second input polynomial to be subtraced from first input polynomial
*/
...
void ML_DSA_87_Poly_Shiftl(poly_D5 *const a)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    a->coeffs[i] = (sint32)((uint32)((uint32)(a->coeffs[i]) << D_ML_DSA));
  }
} // end: ML_DSA_87_Poly_Shiftl
/*====================================================================================================================*/
/**
...
{
  ML_DSA_InvnttTomont(a->coeffs);
} // end: ML_DSA_87_Poly_InvnttTomont
/*====================================================================================================================*/
/**
...
* \param[out] poly_D5       *const c : pointer to output polynomial
* \param[in]  const poly_D5 *const a : pointer to first input polynomial
* \param[in]  const poly_D5 *const b : pointer to second input polynomial
*/
void ML_DSA_87_Poly_PointwiseMontgomery(poly_D5 *const c, const poly_D5 *const a, const poly_D5 *const b)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    c->coeffs[i] = ML_DSA_MontgomeryReduce((sint64)a->coeffs[i] * b->coeffs[i]);
  }
} // end:  ML_DSA_87_Poly_PointwiseMontgomery
/*====================================================================================================================*/
/**
...
*
* \param[out] poly_D5      *const a1 : pointer to output polynomial with coefficients c1
* \param[out] poly_D5            *a0 : pointer to output polynomial with coefficients c0
* \param[in]  const poly_D5 *const a : pointer to input polynomial
*/
void ML_DSA_87_Poly_Power2Round(poly_D5 *const a1, poly_D5 *a0, const poly_D5 *const a)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    a1->coeffs[i] = ML_DSA_87_Power2Round(&a0->coeffs[i], a->coeffs[i]);
  }
} // end: ML_DSA_87_Poly_Power2Round
/*====================================================================================================================*/
/**
...
void ML_DSA_87_Poly_Decompose(poly_D5 *const a1, poly_D5 *a0, const poly_D5 *const a)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    a1->coeffs[i] = ML_DSA_87_Decompose(&a0->coeffs[i], a->coeffs[i]);
  }
} // end: ML_DSA_87_Poly_Decompose
/*====================================================================================================================*/
/**
...
*
* \param[out] poly_D5        *const h : pointer to output hint polynomial
* \param[in]  const poly_D5 *const a0 : pointer to low part of input polynomial
* \param[in]  const poly_D5 *const a1 : pointer to high part of input polynomial
*
...
uint32 ML_DSA_87_Poly_MakeHint(poly_D5 *const h, const poly_D5 *const a0, const poly_D5 *const a1)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    h->coeffs[i] = (sint32)ML_DSA_87_MakeHint(a0->coeffs[i], a1->coeffs[i]);
    s            = s + (uint32)(h->coeffs[i]);
  }

  return s;
} // end: ML_DSA_87_Poly_MakeHint
/*====================================================================================================================*/
/**
* \brief Use hint polynomial to correct the high bits of a polynomial.
*
* \param[out] poly_D5       *const b : pointer to output polynomial with corrected high bits
* \param[in]  const poly_D5 *const a : pointer to input polynomial
* \param[in]  const poly_D5 *const h : pointer to input hint polynomial
*/
void ML_DSA_87_Poly_UseHint(poly_D5 *const b, const poly_D5 *const a, const poly_D5 *const h)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    b->coeffs[i] = ML_DSA_87_UseHint(a->coeffs[i], (uint32)h->coeffs[i]);
  }
} // end: ML_DSA_87_Poly_UseHint
/*====================================================================================================================*/
/**
* \brief Check infinity norm of polynomial against given bound.
*              Assumes input coefficients were reduced by ML_DSA_87_reduce32().
*
* \param[in] const poly_D5 *const a : pointer to polynomial
...
void ML_DSA_87_Poly_Uniform(poly_D5 *a, const uint8 seed[SEEDBYTES_ML_DSA], uint16 nonce)
{
  ...
  ML_DSA_stream128_state state                         = {{0}};

  ML_DSA_Shake128_StreamInit(&state, seed, nonce);
  FsmSw_Fips202_Shake128_IncSqueeze(buf, POLY_UNIFORM_NBLOCKS * SHAKE128_RATE, &state);

  ctr = fsmsw_ml_dsa_87_RejUniform(a->coeffs, N_ML_DSA, buf, buflen);
  ...
  while (ctr < N_ML_DSA)
  {
    ...
    ctr += fsmsw_ml_dsa_87_RejUniform(&a->coeffs[ctr], N_ML_DSA - ctr, buf, buflen);
  }
} // end: ML_DSA_87_Poly_Uniform
/*====================================================================================================================*/
/**
...
*
* \param[out] poly_D5         *a : pointer to output polynomial
* \param[in]  const uint8 seed[] : byte array with seed of length CRHBYTES_ML_DSA
* \param[in]  uint16       nonce : 2-byte nonce
*/
void ML_DSA_87_Poly_UniformEta(poly_D5 *a, const uint8 seed[CRHBYTES_ML_DSA], uint16 nonce)
{
  ...
  ML_DSA_stream256_state state                      = {{0}};

  ML_DSA_Shake256_StreamInit(&state, seed, nonce);
  FsmSw_Fips202_Shake256_IncSqueeze(buf, SHAKE256_RATE, &state);

  ctr = fsmsw_ml_dsa_87_RejEta(a->coeffs, N_ML_DSA, buf, buflen);

  while (ctr < N_ML_DSA)
  {
    FsmSw_Fips202_Shake256_IncSqueeze(buf, SHAKE256_RATE, &state);
    ctr += fsmsw_ml_dsa_87_RejEta(&(a->coeffs[ctr]), N_ML_DSA - ctr, buf, STREAM256_BLOCKBYTES);
  }
} // end: ML_DSA_87_Poly_UniformEta
/*====================================================================================================================*/
/**
* \brief Sample polynomial with uniformly random coefficients in [-(GAMMA1 - 1), GAMMA1] by unpacking
*              output stream of SHAKE256(seed|nonce)
*
* \param[out] poly_D5   *const a : pointer to output polynomial
* \param[in]  const uint8 seed[] : byte array with seed of length CRHBYTES_ML_DSA
...
void ML_DSA_87_Poly_UniformGamma1(poly_D5 *const a, const uint8 seed[CRHBYTES_ML_DSA], uint16 nonce)
{
  ...
  ML_DSA_87_Poly_ZUnpack(a, buf);
} // end: ML_DSA_87_Poly_UniformGamma1
/*====================================================================================================================*/
/**
...
*
* \param[out] poly_D5   *const c : pointer to output polynomial
* \param[in]  const uint8 seed[] : byte array containing challenge of length CTILDEBYTES_ML_DSA_87
*/
void ML_DSA_87_Poly_Challenge(poly_D5 *const c, const uint8 seed[CTILDEBYTES_ML_DSA_87])
{
  ...
  FsmSw_Fips202_Shake256_IncAbsorb(&state, seed, CTILDEBYTES_ML_DSA_87);
  ...
  for (i = N_ML_DSA - TAU_ML_DSA_87; i < N_ML_DSA; ++i)
  {
    do
    {
...
void ML_DSA_87_Polyeta_EtaPack(uint8 *const r, const poly_D5 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 8u); ++i)
  {
    t[0] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[8u * i]);
    t[1] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 1u]);
    t[2] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 2u]);
    t[3] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 3u]);
    t[4] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 4u]);
    t[5] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 5u]);
    t[6] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 6u]);
    t[7] = (uint8)(ETA_ML_DSA_87 - (uint8)a->coeffs[(8u * i) + 7u]);

    r[3u * i]        = (t[0] >> 0) | (t[1] << 3) | (t[2] << 6);
    r[(3u * i) + 1u] = (t[2] >> 2) | (t[3] << 1) | (t[4] << 4) | (t[5] << 7);
    r[(3u * i) + 2u] = (t[5] >> 1) | (t[6] << 2) | (t[7] << 5);
  }
} // end: ML_DSA_87_Polyeta_EtaPack
/*====================================================================================================================*/
/**
* \brief Unpack polynomial with coefficients in [-ETA,ETA].
*
* \param[out] poly_D5     *const r : pointer to output polynomial
...
void ML_DSA_87_Polyeta_EtaUnpack(poly_D5 *const r, const uint8 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 8u); ++i)
  {
    ...
    r->coeffs[8u * i]        = (sint32)ETA_ML_DSA_87 - r->coeffs[8u * i];
    r->coeffs[(8u * i) + 1u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 1u];
    r->coeffs[(8u * i) + 2u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 2u];
    r->coeffs[(8u * i) + 3u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 3u];
    r->coeffs[(8u * i) + 4u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 4u];
    r->coeffs[(8u * i) + 5u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 5u];
    r->coeffs[(8u * i) + 6u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 6u];
    r->coeffs[(8u * i) + 7u] = (sint32)ETA_ML_DSA_87 - r->coeffs[(8u * i) + 7u];
  }
} // end: ML_DSA_87_Polyeta_EtaUnpack
/*====================================================================================================================*/
/**
...
} // end: ML_DSA_87_Poly_T1Unpack
/*====================================================================================================================*/
/**
* \brief Bit-pack polynomial t0 with coefficients in [-2^{D-1}, 2^{D-1}].
*
* \param[out] uint8         *const r : pointer to output byte array with at least POLYT0_PACKEDBYTES_ML_DSA bytes
* \param[in]  const poly_D5 *const a : pointer to input polynomial
*/
void ML_DSA_87_Poly_T0Pack(uint8 *const r, const poly_D5 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 8u); ++i)
  {
    t[0] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[8u * i]);
    t[1] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 1u]);
    t[2] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 2u]);
    t[3] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 3u]);
    t[4] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 4u]);
    t[5] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 5u]);
    t[6] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 6u]);
    t[7] = ((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)a->coeffs[(8u * i) + 7u]);
...
void ML_DSA_87_Poly_T0Unpack(poly_D5 *const r, const uint8 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 8u); ++i)
  {
    ...
    r->coeffs[8u * i] = (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[8u * i])));
    r->coeffs[(8u * i) + 1u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 1u])));
    r->coeffs[(8u * i) + 2u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 2u])));
    r->coeffs[(8u * i) + 3u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 3u])));
    r->coeffs[(8u * i) + 4u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 4u])));
    r->coeffs[(8u * i) + 5u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 5u])));
    r->coeffs[(8u * i) + 6u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 6u])));
    r->coeffs[(8u * i) + 7u] =
        (sint32)((uint32)(((uint32)1u << (D_ML_DSA - 1u)) - ((uint32)r->coeffs[(8u * i) + 7u])));
  }
} // end: ML_DSA_87_Poly_T0Unpack
/*====================================================================================================================*/
/**
* \brief Bit-pack polynomial with coefficients in [-(GAMMA1 - 1), GAMMA1].
*
* \param[out] uint8         *const r : pointer to output byte array with at least POLYZ_PACKEDBYTES_ML_DSA_87 bytes
* \param[in]  const poly_D5 *const a : pointer to input polynomial
*/
void ML_DSA_87_Poly_ZPack(uint8 *const r, const poly_D5 *const a)
{
  uint16 i = 0;
  /* polyspace +3 DEFECT:PARTIALLY_ACCESSED_ARRAY [To fix:]"Functionality of ML_DSA_65_Poly_ZPack and   
     ML_DSA_87_Poly_ZPack (only the first two elements of the array t are written) differs from that of  
     ML_DSA_44_Poly_ZPack. Fix it after response from github.com/PQClean/PQClean/discussions/605" */
  uint32 t[POLY_ZPACK_BUFFER_SIZE] = {0};

  for (i = 0; i < (N_ML_DSA / 2u); ++i)
  {
    t[0] = GAMMA1_ML_DSA_87 - (uint32)a->coeffs[2u * i];
    t[1] = GAMMA1_ML_DSA_87 - (uint32)a->coeffs[(2u * i) + 1u];
...
void ML_DSA_87_Poly_ZUnpack(poly_D5 *const r, const uint8 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 2u); ++i)
  {
    ...
    /* Known issue: This row is maybe not correct. See https://github.com/pq-crystals/ml_dsa/issues/63 */
    /* r->coeffs[2u * i + 1] = (sint32)((uint32)((uint32)r->coeffs[2u * i] & 0xFFFFFu)); */ /* No effect, since we're anyway at 20 bits */

    r->coeffs[2u * i]        = (sint32)((uint32)(GAMMA1_ML_DSA_87 - (uint32)r->coeffs[2u * i]));
    r->coeffs[(2u * i) + 1u] = (sint32)((uint32)(GAMMA1_ML_DSA_87 - (uint32)r->coeffs[(2u * i) + 1u]));
  }
} // end: ML_DSA_87_Poly_ZUnpack
/*====================================================================================================================*/
/**
* \brief Bit-pack polynomial w1 with coefficients in [0,15] or [0,43].
*              Input coefficients are assumed to be standard representatives.
*
* \param[out] uint8         *const r : pointer to output byte array with at least POLYW1_PACKEDBYTES_ML_DSA_87  bytes
* \param[in]  const poly_D5 *const a : pointer to input polynomial
*/
