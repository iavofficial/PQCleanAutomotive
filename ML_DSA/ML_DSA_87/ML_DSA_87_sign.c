 ...
 ** @{ */
/*====================================================================================================================*/
/** \file ML_DSA_87_sign.c
* \brief  description of ML_DSA_87_sign.c
*
* \details
*
*
...
*
* \param[out] uint8     *sig    : pointer to output signature (of length FSMSW_ML_DSA_87_CRYPTO_BYTES)
                                                 ...
                                                 uint32 mlen, const uint8 *ctx, uint32 ctxlen, const uint8 *const sk)
{
  ...
  else
  {
    ...
    ML_DSA_87_UnpackSk(rho, tr, key, &t0, &s1, &s2, sk);
    ...
    ML_DSA_87_Polyvec_MatrixExpand(mat, rho);
    ML_DSA_87_Polyvecl_Ntt(&s1);
    ML_DSA_87_Polyveck_Ntt(&s2);
    ML_DSA_87_Polyveck_Ntt(&t0);

    while (TRUE == loop)
    {
      /* Sample intermediate vector y */
      ML_DSA_87_Polyvecl_UniformGamma1(&y, rhoprime, nonce);
      nonce++;

      /* Matrix-vector multiplication */
      z = y;
      ML_DSA_87_Polyvecl_Ntt(&z);
      ML_DSA_87_Polyvec_MatrixPointwiseMontgomery(&w1, mat, &z);
      ML_DSA_87_Polyveck_Reduce(&w1);
      ML_DSA_87_Polyveck_InvnttTomont(&w1);

      /* Decompose w and call the random oracle */
      ML_DSA_87_Polyveck_CAddQ(&w1);
      ML_DSA_87_Polyveck_Decompose(&w1, &w0, &w1);
      ML_DSA_87_Polyveck_PackW1(sig, &w1);

      FsmSw_Fips202_Shake256_IncInit(&state);
      FsmSw_Fips202_Shake256_IncAbsorb(&state, mu, CRHBYTES_ML_DSA);
      FsmSw_Fips202_Shake256_IncAbsorb(&state, sig, K_ML_DSA_87 * POLYW1_PACKEDBYTES_ML_DSA_87);
      FsmSw_Fips202_Shake256_IncFinalize(&state);
      FsmSw_Fips202_Shake256_IncSqueeze(sig, CTILDEBYTES_ML_DSA_87, &state);
      ML_DSA_87_Poly_Challenge(&cp, sig);
      ML_DSA_87_Poly_Ntt(&cp);

      /* Compute z, reject if it reveals secret */
      ML_DSA_87_Polyvecl_PointwisePolyMontgomery(&z, &cp, &s1);
      ML_DSA_87_Polyvecl_InvnttTomont(&z);
      ML_DSA_87_Polyvecl_Add(&z, &z, &y);
      ML_DSA_87_Polyvecl_Reduce(&z);
      if (0 < ML_DSA_87_Polyvecl_Chknorm(&z, (sint32)(GAMMA1_ML_DSA_87 - BETA_ML_DSA_87)))
      {
        continue;
      }
      ...
      ML_DSA_87_Polyveck_PointwisePolyMontgomery(&h, &cp, &s2);
      ML_DSA_87_Polyveck_InvnttTomont(&h);
      ML_DSA_87_Polyveck_Sub(&w0, &w0, &h);
      ML_DSA_87_Polyveck_Reduce(&w0);
      if (0 < ML_DSA_87_Polyveck_Chknorm(&w0, (sint32)((uint32)((uint32)GAMMA2_ML_DSA_87 - BETA_ML_DSA_87))))
      {
        continue;
      }

      /* Compute hints for w1 */
      ML_DSA_87_Polyveck_PointwisePolyMontgomery(&h, &cp, &t0);
      ML_DSA_87_Polyveck_InvnttTomont(&h);
      ML_DSA_87_Polyveck_Reduce(&h);
      if (0 < ML_DSA_87_Polyveck_Chknorm(&h, (sint32)GAMMA2_ML_DSA_87))
      {
        continue;
      }

      ML_DSA_87_Polyveck_Add(&w0, &w0, &h);
      n = ML_DSA_87_Polyveck_MakeHint(&h, &w0, &w1);
      if (n > OMEGA_ML_DSA_87)
      {
        continue;
      }
    ...
    /* Write signature */
    ML_DSA_87_PackSig(sig, sig, &z, &h);
    *siglen = FSMSW_ML_DSA_87_CRYPTO_BYTES;
  }
...
*                                 array with FSMSW_ML_DSA_87_CRYPTO_BYTES + mlen bytes), can be equal to m
                                       ...
                                       const uint8 *ctx, uint32 ctxlen, const uint8 *const sk)
{
  ...
  for (i = 0u; i < mlen; ++i)
  {
    sm[FSMSW_ML_DSA_87_CRYPTO_BYTES + mlen - 1u - i] = m[mlen - 1u - i];
  }
  retVal =
      ML_DSA_87_Crypto_Sign_Signature_Ctx(sm, smlen, &sm[FSMSW_ML_DSA_87_CRYPTO_BYTES], mlen, ctx, ctxlen, sk);
  *smlen += mlen;
  return retVal;
}
...
{
  uint16 i                                                = 0u;
  uint8 buf[K_ML_DSA_87 * POLYW1_PACKEDBYTES_ML_DSA_87] = {0u};
  uint8 rho[SEEDBYTES_ML_DSA]                          = {0u};
  uint8 mu[CRHBYTES_ML_DSA]                            = {0u};
  uint8 c[CTILDEBYTES_ML_DSA_87]                         = {0u};
  uint8 c2[CTILDEBYTES_ML_DSA_87]                        = {0u};
  poly_D5 cp                                              = {{0}};
  polyvecl_D5 mat[K_ML_DSA_87]                           = {{{{{0}}}}};
  polyvecl_D5 z                                           = {{{{0}}}};
  polyveck_D5 t1                                          = {{{{0}}}};
  ...
  if ((ctxlen > 255) || (siglen != FSMSW_ML_DSA_87_CRYPTO_BYTES))
  {
    retVal = ERR_NOT_OK;
  }

  ML_DSA_87_UnpackPk(rho, &t1, pk);
  if (0 < ML_DSA_87_UnpackSig(c, &z, &h, sig))
  {
    retVal = ERR_NOT_OK;
  }
  if (0 < ML_DSA_87_Polyvecl_Chknorm(&z, (sint32)(GAMMA1_ML_DSA_87 - BETA_ML_DSA_87)))
  ...
  FsmSw_Fips202_Shake256(mu, TRBYTES_ML_DSA, pk, FSMSW_ML_DSA_87_CRYPTO_PUBLICKEYBYTES);
  FsmSw_Fips202_Shake256_IncInit(&state);
  ...
  ML_DSA_87_Poly_Ntt(&cp);
  ML_DSA_87_Polyveck_Shiftl(&t1);
  ML_DSA_87_Polyveck_Ntt(&t1);
  ML_DSA_87_Polyveck_PointwisePolyMontgomery(&t1, &cp, &t1);

  ML_DSA_87_Polyveck_Sub(&w1, &w1, &t1);
  ML_DSA_87_Polyveck_Reduce(&w1);
  ML_DSA_87_Polyveck_InvnttTomont(&w1);

  /* Reconstruct w1 */
  ML_DSA_87_Polyveck_CAddQ(&w1);
  ML_DSA_87_Polyveck_UseHint(&w1, &w1, &h);
  ML_DSA_87_Polyveck_PackW1(buf, &w1);

  /* Call random oracle and verify challenge */
  FsmSw_Fips202_Shake256_IncInit(&state);
  FsmSw_Fips202_Shake256_IncAbsorb(&state, mu, CRHBYTES_ML_DSA);
  FsmSw_Fips202_Shake256_IncAbsorb(&state, buf, K_ML_DSA_87 * POLYW1_PACKEDBYTES_ML_DSA_87);
  FsmSw_Fips202_Shake256_IncFinalize(&state);
  FsmSw_Fips202_Shake256_IncSqueeze(c2, CTILDEBYTES_ML_DSA_87, &state);
  for (i = 0u; i < CTILDEBYTES_ML_DSA_87; ++i)
  {
    if (c[i] != c2[i])
    {
      retVal = ERR_NOT_OK;
                                            ...
                                            const uint8 *ctx, uint32 ctxlen, const uint8 *const pk)
{
  ...
  if (smlen >= FSMSW_ML_DSA_87_CRYPTO_BYTES)
  {
    *mlen = smlen - FSMSW_ML_DSA_87_CRYPTO_BYTES;
    if (0u == ML_DSA_87_Crypto_Sign_Verify_Ctx(sm, FSMSW_ML_DSA_87_CRYPTO_BYTES,
                                                      &sm[FSMSW_ML_DSA_87_CRYPTO_BYTES], *mlen, ctx, ctxlen, pk))
    {
      /* All good, copy msg, return 0u */
      for (i = 0u; i < *mlen; ++i)
      {
        m[i] = sm[FSMSW_ML_DSA_87_CRYPTO_BYTES + i];
      }
      retVal = ERR_OK;
    }
  }
 ...
 *                           (allocated array of FSMSW_ML_DSA_87_CRYPTO_PUBLICKEYBYTES bytes)
 * \param[out] uint8 *sk : pointer to output private key (allocated
 *                           array of FSMSW_ML_DSA_87_CRYPTO_SECRETKEYBYTES bytes)
 */
void ML_DSA_87_Crypto_Sign_KeyPair(uint8 *const pk, uint8 *const sk)
{
  ...
  seedbuf[SEEDBYTES_ML_DSA]     = K_ML_DSA_87;
  seedbuf[SEEDBYTES_ML_DSA + 1] = L_ML_DSA_87;
  ...
  ML_DSA_87_Polyvecl_UniformEta(&s1, rhoprime, 0u);
  ML_DSA_87_Polyveck_UniformEta(&s2, rhoprime, L_ML_DSA_87);

  /* Matrix-vector multiplication */
  s1hat = s1;
  ML_DSA_87_Polyvecl_Ntt(&s1hat);
  ML_DSA_87_Polyvec_MatrixPointwiseMontgomery(&t1, mat, &s1hat);
  ML_DSA_87_Polyveck_Reduce(&t1);
  ML_DSA_87_Polyveck_InvnttTomont(&t1);

  /* Add error vector s2 */
  ML_DSA_87_Polyveck_Add(&t1, &t1, &s2);

  /* Extract t1 and write public key */
  ML_DSA_87_Polyveck_CAddQ(&t1);
  ML_DSA_87_Polyveck_Power2Round(&t1, &t0, &t1);
  ML_DSA_87_PackPk(pk, rho, &t1);

  /* Compute H(rho, t1) and write secret key */
  FsmSw_Fips202_Shake256(tr, TRBYTES_ML_DSA, pk, FSMSW_ML_DSA_87_CRYPTO_PUBLICKEYBYTES);
  ML_DSA_87_PackSk(sk, rho, tr, key, &t0, &s1, &s2);

  return;
} // end: ML_DSA_87_Crypto_Sign_KeyPair
/*====================================================================================================================*/
/**
...
{
  return ML_DSA_87_Crypto_Sign_Signature_Ctx(sig, siglen, m, mlen, NULL_PTR, 0, sk);
} // end: ML_DSA_87_Crypto_Sign_Signature
/*====================================================================================================================*/
/**
...
uint8 ML_DSA_87_Crypto_Sign(uint8 *const sm, uint32 *const smlen, const uint8 *const m, uint32 mlen,
                                   const uint8 *const sk)
{
  return ML_DSA_87_Crypto_Sign_Ctx(sm, smlen, m, mlen, NULL_PTR, 0, sk);
} // end: ML_DSA_87_Crypto_Sign
/*====================================================================================================================*/
/**
...
{
  return ML_DSA_87_Crypto_Sign_Verify_Ctx(sig, siglen, m, mlen, NULL_PTR, 0, pk);
} // end: ML_DSA_87_Crypto_Sign_Verify
/*====================================================================================================================*/
/**
...
uint8 ML_DSA_87_Crypto_Sign_Open(uint8 *const m, uint32 *const mlen, const uint8 *const sm, uint32 smlen,
                                        const uint8 *const pk)
{
  return ML_DSA_87_Crypto_Sign_Open_Ctx(m, mlen, sm, smlen, NULL_PTR, 0, pk);
} // end: ML_DSA_87_Crypto_Sign_Open
