...
/*====================================================================================================================*/
/** \file ML_DSA_65_poly.c
* \brief  description of ML_DSA_65_poly.c
*
* \details
*
*
...
#define POLY_UNIFORM_GAMMA1_NBLOCKS ((POLYZ_PACKEDBYTES_ML_DSA_65 + STREAM256_BLOCKBYTES - 1u) / STREAM256_BLOCKBYTES)
/**********************************************************************************************************************/
/* TYPES                                                                                                              */
/**********************************************************************************************************************/
...
/**********************************************************************************************************************/
static uint32 fsmsw_ml_dsa_65_RejEta(sint32 *const a, uint32 len, const uint8 *const buf, uint32 buflen);
static uint32 fsmsw_ml_dsa_65_RejUniform(sint32 *const a, uint32 len, const uint8 *const buf, uint32 buflen);
/**********************************************************************************************************************/
...
static uint32 fsmsw_ml_dsa_65_RejUniform(sint32 *const a, uint32 len, const uint8 *const buf, uint32 buflen)
{
  ...
  return ctr;
} // end: fsmsw_ml_dsa_65_RejUniform
...
} // end: ML_DSA_65_Poly_InvnttTomont
/*====================================================================================================================*/
/**
...
} // end: ML_DSA_65_Poly_PointwiseMontgomery
/*====================================================================================================================*/
/**
...
void ML_DSA_65_Poly_Power2Round(poly_D3 *const a1, poly_D3 *a0, const poly_D3 *const a)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    a1->coeffs[i] = ML_DSA_65_Power2Round(&a0->coeffs[i], a->coeffs[i]);
  }
} // end: ML_DSA_65_Poly_Power2Round
/*====================================================================================================================*/
/**
...
void ML_DSA_65_Poly_Decompose(poly_D3 *const a1, poly_D3 *a0, const poly_D3 *const a)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    a1->coeffs[i] = ML_DSA_65_Decompose(&a0->coeffs[i], a->coeffs[i]);
  }
} // end: ML_DSA_65_Poly_Decompose
/*====================================================================================================================*/
/**
...
uint32 ML_DSA_65_Poly_MakeHint(poly_D3 *const h, const poly_D3 *const a0, const poly_D3 *const a1)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    h->coeffs[i] = (sint32)ML_DSA_65_MakeHint(a0->coeffs[i], a1->coeffs[i]);
    s            = s + (uint32)(h->coeffs[i]);
  }

  return s;
} // end: ML_DSA_65_Poly_MakeHint
/*====================================================================================================================*/
/**
...
void ML_DSA_65_Poly_UseHint(poly_D3 *const b, const poly_D3 *const a, const poly_D3 *const h)
{
  ...
  for (i = 0; i < N_ML_DSA; ++i)
  {
    b->coeffs[i] = ML_DSA_65_UseHint(a->coeffs[i], (uint32)h->coeffs[i]);
  }
} // end: ML_DSA_65_Poly_UseHint
/*====================================================================================================================*/
/**
* \brief Check infinity norm of polynomial against given bound.
*              Assumes input coefficients were reduced by ML_DSA_65_reduce32().
*
...
void ML_DSA_65_Poly_Uniform(poly_D3 *a, const uint8 seed[SEEDBYTES_ML_DSA], uint16 nonce)
{
  ...
  ctr = fsmsw_ml_dsa_65_RejUniform(a->coeffs, N_ML_DSA, buf, buflen);
  ...
  while (ctr < N_ML_DSA)
  {
    ...
    ctr += fsmsw_ml_dsa_65_RejUniform(&a->coeffs[ctr], N_ML_DSA - ctr, buf, buflen);
  }
} // end: ML_DSA_65_Poly_Uniform
/*====================================================================================================================*/
/**
...
void ML_DSA_65_Poly_UniformEta(poly_D3 *a, const uint8 seed[CRHBYTES_ML_DSA], uint16 nonce)
{
  ...
  ctr = fsmsw_ml_dsa_65_RejEta(a->coeffs, N_ML_DSA, buf, buflen);

  while (ctr < N_ML_DSA)
  {
    FsmSw_Fips202_Shake256_IncSqueeze(buf, SHAKE256_RATE, &state);
    ctr += fsmsw_ml_dsa_65_RejEta(&(a->coeffs[ctr]), N_ML_DSA - ctr, buf, STREAM256_BLOCKBYTES);
  }
} // end: ML_DSA_65_Poly_UniformEta
/*====================================================================================================================*/
/**
...
void ML_DSA_65_Poly_UniformGamma1(poly_D3 *const a, const uint8 seed[CRHBYTES_ML_DSA], uint16 nonce)
{
  ...
  ML_DSA_65_Poly_ZUnpack(a, buf);
} // end: ML_DSA_65_Poly_UniformGamma1
/*====================================================================================================================*/
/**
...
*
* \param[out] poly_D3 *const c   : pointer to output polynomial
* \param[in]  const uint8 seed[] : byte array containing challenge of length CTILDEBYTES_ML_DSA_65
*/
void ML_DSA_65_Poly_Challenge(poly_D3 *const c, const uint8 seed[CTILDEBYTES_ML_DSA_65])
{
  ...
  FsmSw_Fips202_Shake256_IncAbsorb(&state, seed, CTILDEBYTES_ML_DSA_65);
  ...
  for (i = N_ML_DSA - TAU_ML_DSA_65; i < N_ML_DSA; ++i)
  {
    do
    {
...
{
  uint16 i = 0;
  /* polyspace +3 DEFECT:PARTIALLY_ACCESSED_ARRAY [To fix:]"Functionality of ML_DSA_65_Polyeta_EtaPack differs 
     (only the first two elements of the array t are written) from that of ML_DSA_44_Polyeta_EtaPack and 
     ML_DSA_87_Polyeta_EtaPack. Fix it after response from github.com/PQClean/PQClean/discussions/605" */
  uint8 t[POLY_ETA_BUFFER_SIZE] = {0};

  for (i = 0; i < (N_ML_DSA / 2u); ++i)
  {
    t[0] = (uint8)(ETA_ML_DSA_65 - (uint8)a->coeffs[2u * i]);
    t[1] = (uint8)(ETA_ML_DSA_65 - (uint8)a->coeffs[(2u * i) + 1u]);
    r[i] = t[0] | (t[1] << 4);
  }
} // end: ML_DSA_65_Polyeta_EtaPack
/*====================================================================================================================*/
/**
...
void ML_DSA_65_Polyeta_EtaUnpack(poly_D3 *const r, const uint8 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 2u); ++i)
  {
    ...
    r->coeffs[2u * i]        = (sint32)ETA_ML_DSA_65 - r->coeffs[2u * i];
    r->coeffs[(2u * i) + 1u] = (sint32)ETA_ML_DSA_65 - r->coeffs[(2u * i) + 1u];
  }
} // end: ML_DSA_65_Polyeta_EtaUnpack
/*====================================================================================================================*/
/**
...
} // end: ML_DSA_65_Poly_T0Unpack
/*====================================================================================================================*/
/**
* \brief Bit-pack polynomial with coefficients in [-(GAMMA1 - 1), GAMMA1].
*
* \param[out] uint8         *const r : pointer to output byte array with at least POLYZ_PACKEDBYTES_ML_DSA_65 bytes
* \param[in]  const poly_D3 *const a : pointer to input polynomial
*/
void ML_DSA_65_Poly_ZPack(uint8 *const r, const poly_D3 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 2u); ++i)
  {
    t[0] = GAMMA1_ML_DSA_65 - (uint32)a->coeffs[2u * i];
    t[1] = GAMMA1_ML_DSA_65 - (uint32)a->coeffs[(2u * i) + 1u];
...
void ML_DSA_65_Poly_ZUnpack(poly_D3 *const r, const uint8 *const a)
{
  ...
  for (i = 0; i < (N_ML_DSA / 2u); ++i)
  {
    ...
    r->coeffs[2u * i]        = (sint32)((uint32)(GAMMA1_ML_DSA_65 - (uint32)r->coeffs[2u * i]));
    r->coeffs[(2u * i) + 1u] = (sint32)((uint32)(GAMMA1_ML_DSA_65 - (uint32)r->coeffs[(2u * i) + 1u]));
  }
} // end: ML_DSA_65_Poly_ZUnpack
/*====================================================================================================================*/
/**
* \brief Bit-pack polynomial w1 with coefficients in [0,15] or [0,43].
*              Input coefficients are assumed to be standard representatives.
*
* \param[out] uint8         *const r : pointer to output byte array with at least POLYW1_PACKEDBYTES_ML_DSA_65 bytes
* \param[in]  const poly_D3 *const a : pointer to input polynomial
*/
