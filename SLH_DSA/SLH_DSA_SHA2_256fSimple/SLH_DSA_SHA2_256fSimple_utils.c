                                              ...
                                              const slh_dsa_sha2_256f_ctx *const ctx, uint32 addr[8])
{
  ...
  {
    FsmSw_CommonLib_MemCpy(&buffer[SLH_DSA_SHA2_256FSIMPLE_N], leaf, SLH_DSA_SHA2_256FSIMPLE_N);
    FsmSw_CommonLib_MemCpy(buffer, auth_path_temp, SLH_DSA_SHA2_256FSIMPLE_N);
  }
  else
  {
    FsmSw_CommonLib_MemCpy(buffer, leaf, SLH_DSA_SHA2_256FSIMPLE_N);
    FsmSw_CommonLib_MemCpy(&buffer[SLH_DSA_SHA2_256FSIMPLE_N], auth_path_temp, SLH_DSA_SHA2_256FSIMPLE_N);
  }
                                           ...
                                           uint32 tree_addr[8])
{
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    {
      FsmSw_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * SLH_DSA_SHA2_256FSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      {
        FsmSw_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * SLH_DSA_SHA2_256FSIMPLE_N],
                               &stack[(offset - 1u) * SLH_DSA_SHA2_256FSIMPLE_N], SLH_DSA_SHA2_256FSIMPLE_N);
      }
    }
  }
  FsmSw_CommonLib_MemCpy(root, stack, SLH_DSA_SHA2_256FSIMPLE_N);
