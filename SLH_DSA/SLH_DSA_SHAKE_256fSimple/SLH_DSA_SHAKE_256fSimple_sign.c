 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHAKE_256fSimple_sign
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_256fSimple_sign.c
* \brief  description of SLH_DSA_SHAKE_256fSimple_sign.c
*
* \details
*
*
...
#include "SLH_DSA_SHAKE_256fSimple_FctWrapper.h"
#include "SLH_DSA_SHAKE_256fSimple_context.h"
#include "SLH_DSA_SHAKE_256fSimple_fors.h"
#include "SLH_DSA_SHAKE_256fSimple_hash.h"
#include "SLH_DSA_SHAKE_256fSimple_merkle.h"
...
#include "SLH_DSA_SHAKE_256fSimple_wots.h"
#include "SLH_DSA_SHAKE_address.h"

#include "SLH_DSA_SHAKE_256fSimple_sign.h"
/**********************************************************************************************************************/
/* DEFINES                                                                                                            */
/**********************************************************************************************************************/
...
/**********************************************************************************************************************/
static void slh_dsa_shake_256fsimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
...
static void slh_dsa_shake_256fsimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                  const uint8 *const seed)
{
  slh_dsa_shake_256f_ctx ctx = {{0}};

  /* Initialize SK_SEED, SK_PRF and PUB_SEED from seed. */
  SLH_DSA_CommonLib_MemCpy(sk, seed, SLH_DSA_SHAKE_256FSIMPLE_CRYPTO_SEEDBYTES);

  SLH_DSA_CommonLib_MemCpy(pk, &sk[2u * SLH_DSA_SHAKE_256FSIMPLE_N], SLH_DSA_SHAKE_256FSIMPLE_N);

  SLH_DSA_CommonLib_MemCpy(ctx.pub_seed, pk, SLH_DSA_SHAKE_256FSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(ctx.sk_seed, sk, SLH_DSA_SHAKE_256FSIMPLE_N);

  /* This hook allows the hash function instantiation to do whatever
     preparation or computation it needs, based on the public seed. */
  SLH_DSA_SHAKE_256fSimple_InitializeHashFunction(&ctx);

  /* Compute root node of the top-most subtree. */
  SLH_DSA_SHAKE_256fSimple_Merkle_GenRoot(&sk[3u * SLH_DSA_SHAKE_256FSIMPLE_N], &ctx);

  /* cleanup */
  SLH_DSA_SHAKE_256fSimple_1FreeHashFunction(&ctx);

  SLH_DSA_CommonLib_MemCpy(&pk[SLH_DSA_SHAKE_256FSIMPLE_N], &sk[3u * SLH_DSA_SHAKE_256FSIMPLE_N],
                         SLH_DSA_SHAKE_256FSIMPLE_N);

  return;
} // end: slh_dsa_shake_256fsimple_crypto_sign_SeedKeyPair
/**********************************************************************************************************************/
...
{
  uint8 seed[SLH_DSA_SHAKE_256FSIMPLE_CRYPTO_SEEDBYTES] = {0};
  (void)SLH_DSA_CommonLib_RandomBytes(seed, SLH_DSA_SHAKE_256FSIMPLE_CRYPTO_SEEDBYTES);
  slh_dsa_shake_256fsimple_crypto_sign_SeedKeyPair(pk, sk, seed);

  return;
} // end: SLH_DSA_SHAKE_256fSimple_Crypto_Sign_KeyPair
...
{
  slh_dsa_shake_256f_ctx ctx                                = {{0}};
  const uint8 *const sk_prf                                 = &sk[SLH_DSA_SHAKE_256FSIMPLE_N];
  const uint8 *const pk                                     = &sk[2u * SLH_DSA_SHAKE_256FSIMPLE_N];
  uint8 optrand[SLH_DSA_SHAKE_256FSIMPLE_N]            = {0};
  uint8 mhash[SLH_DSA_SHAKE_256FSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[SLH_DSA_SHAKE_256FSIMPLE_N]               = {0};
  uint32 i                                                  = 0;
  uint64 tree                                               = 0;
  uint32 idx_leaf                                           = 0;
  ...
  (void)SLH_DSA_CommonLib_RandomBytes(optrand, SLH_DSA_SHAKE_256FSIMPLE_N);
  /* Compute the digest randomization value. */
  SLH_DSA_SHAKE_256fSimple_GenMessageRandom(sig_temp, sk_prf, optrand, m, mlen, &ctx);

  /* Derive the message digest and leaf index from R, PK and M. */
  SLH_DSA_SHAKE_256fSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_N];

  SLH_DSA_SHAKE_SetTreeAddr(wots_addr, tree);
  SLH_DSA_SHAKE_256fSimple_set_keypair_addr(wots_addr, idx_leaf);

  /* Sign the message hash using FORS. */
  SLH_DSA_SHAKE_256fSimple_Fors_Sign(sig_temp, root, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_FORS_BYTES];

  for (i = 0; i < SLH_DSA_SHAKE_256FSIMPLE_D; i++)
  {
    ...
    SLH_DSA_SHAKE_256fSimple_Merkle_Sign(sig_temp, root, &ctx, wots_addr, tree_addr, idx_leaf);
    sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_WOTS_BYTES +
                         (SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT * SLH_DSA_SHAKE_256FSIMPLE_N)];

    /* Update the indices for the next layer. */
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT)) - 1u));
    tree     = tree >> SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT;
  }

  SLH_DSA_SHAKE_256fSimple_1FreeHashFunction(&ctx);

  *siglen = SLH_DSA_SHAKE_256FSIMPLE_BYTES;

  return;
} // end: SLH_DSA_SHAKE_256fSimple_Crypto_Sign_Signature
...
{
  slh_dsa_shake_256f_ctx ctx                                = {{0}};
  const uint8 *const pub_root                               = &pk[SLH_DSA_SHAKE_256FSIMPLE_N];
  uint8 mhash[SLH_DSA_SHAKE_256FSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[SLH_DSA_SHAKE_256FSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[SLH_DSA_SHAKE_256FSIMPLE_N]               = {0};
  uint8 leaf[SLH_DSA_SHAKE_256FSIMPLE_N]               = {0};
  uint32 i                                                  = 0;
  uint64 tree                                               = 0;
  uint32 idx_leaf                                           = 0;
  uint32 wots_addr[SLH_DSA_SIGN_ADDR_SIZE]            = {0};
  ...
  if (siglen != SLH_DSA_SHAKE_256FSIMPLE_BYTES)
  {
    retVal = ERR_NOT_OK;
  }
  ...
  /* The additional SLH_DSA_SHAKE_256FSIMPLE_N is a result of the hash domain separator. */
  SLH_DSA_SHAKE_256fSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_N];
  ...
  SLH_DSA_SHAKE_256fSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < SLH_DSA_SHAKE_256FSIMPLE_D; i++)
  {
    ...
    SLH_DSA_SHAKE_256fSimple_copy_keypair_addr(wots_pk_addr, wots_addr);
    ...
    SLH_DSA_SHAKE_256fSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_WOTS_BYTES];

    /* Compute the leaf node using the WOTS public key. */
    SLH_DSA_SHAKE_256fSimple_Thash(leaf, wots_pk, SLH_DSA_SHAKE_256FSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

    /* Compute the root node of this subtree. */
    SLH_DSA_SHAKE_256fSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
                                              SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
    sig_temp = &sig_temp[SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT * SLH_DSA_SHAKE_256FSIMPLE_N];
  ...
  if (SLH_DSA_CommonLib_MemCmp(root, pub_root, SLH_DSA_SHAKE_256FSIMPLE_N) != 0u)
  {
    retVal = ERR_NOT_OK;
  }

  return retVal;
} // end: SLH_DSA_SHAKE_256fSimple_Crypto_Sign_Verify
                                               ...
                                               const uint8 *const sk)
{
  ...
  (void)SLH_DSA_SHAKE_256fSimple_Crypto_Sign_Signature(sm, &siglen, m, mlen, sk);

  SLH_DSA_CommonLib_MemMove(&sm[SLH_DSA_SHAKE_256FSIMPLE_BYTES], m, mlen);
  *smlen = siglen + mlen;

  return;
} // end: SLH_DSA_SHAKE_256fSimple_Crypto_Sign
                                                     ...
                                                     uint32 smlen, const uint8 *const pk)
{
   ...
   * exactly SLH_DSA_SHAKE_256FSIMPLE_BYTES. */
  if (smlen < SLH_DSA_SHAKE_256FSIMPLE_BYTES)
  {
  ...
  {
    *mlen = smlen - SLH_DSA_SHAKE_256FSIMPLE_BYTES;

    if (0 != SLH_DSA_SHAKE_256fSimple_Crypto_Sign_Verify(sm, SLH_DSA_SHAKE_256FSIMPLE_BYTES,
                                                              &sm[SLH_DSA_SHAKE_256FSIMPLE_BYTES], *mlen, pk))
    {
      SLH_DSA_CommonLib_MemSet(m, 0, smlen);
      *mlen  = 0;
      retVal = ERR_NOT_OK;
    ...
    SLH_DSA_CommonLib_MemMove(m, &sm[SLH_DSA_SHAKE_256FSIMPLE_BYTES], *mlen);
  }
  return retVal;
} // end: SLH_DSA_SHAKE_256fSimple_Crypto_Sign_Open
