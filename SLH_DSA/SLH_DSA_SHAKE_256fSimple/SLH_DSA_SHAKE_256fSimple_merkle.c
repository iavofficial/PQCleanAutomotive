 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHAKE_256fSimple_merkle
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_256fSimple_merkle.c
* \brief  description of SLH_DSA_SHAKE_256fSimple_merkle.c
*
* \details
*
*
 ...
 *
 */
void SLH_DSA_SHAKE_256fSimple_Merkle_Sign(uint8 *const sig, uint8 *const root,
                                               const slh_dsa_shake_256f_ctx *const ctx, const uint32 wots_addr[8],
                                               uint32 tree_addr[8], uint32 idx_leaf)
{
  uint8 *const auth_path = &sig[SLH_DSA_SHAKE_256FSIMPLE_WOTS_BYTES];

  SLH_DSA_SHAKE_256fSimple_LeafInfoX1_T info = {
      ((void *)0),
  ...
  uint32 steps[SLH_DSA_SHAKE_256FSIMPLE_WOTS_LEN] = {0};

  info.wots_sig = sig;
  SLH_DSA_SHAKE_256fSimple_Wots_ChainLengths(steps, root);
  ...
  SLH_DSA_SHAKE_256fSimple_TreeHashX1(root, auth_path, ctx, idx_leaf, 0, SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT,
                                           SLH_DSA_SHAKE_256fSimple_Wots_GenLeafX1, tree_addr, &info);
} // end: SLH_DSA_SHAKE_256fSimple_Merkle_Sign
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void SLH_DSA_SHAKE_256fSimple_Merkle_GenRoot(uint8 *const root, const slh_dsa_shake_256f_ctx *const ctx)
{
  /* We do not need the auth path in key generation, but it simplifies the code to have just one
   * SLH_DSA_SHAKE_256fSimple_TreeHash routine that computes both root and path in one function. */
  uint8 auth_path[(SLH_DSA_SHAKE_256FSIMPLE_TREE_HEIGHT * SLH_DSA_SHAKE_256FSIMPLE_N) +
                  SLH_DSA_SHAKE_256FSIMPLE_WOTS_BYTES] = {0};
  uint32 top_tree_addr[SLH_DSA_SIGN_ADDR_SIZE]        = {0};
  uint32 wots_addr[SLH_DSA_SIGN_ADDR_SIZE]            = {0};

  SLH_DSA_SHAKE_SetLayerAddr(top_tree_addr, SLH_DSA_SHAKE_256FSIMPLE_D - 1u);
  SLH_DSA_SHAKE_SetLayerAddr(wots_addr, SLH_DSA_SHAKE_256FSIMPLE_D - 1u);

  SLH_DSA_SHAKE_256fSimple_Merkle_Sign(auth_path, root, ctx, wots_addr, top_tree_addr,
                                            ~((uint32)0u) /* ~0 means "don't bother generating an auth path */);
} // end: SLH_DSA_SHAKE_256fSimple_Merkle_GenRoot
