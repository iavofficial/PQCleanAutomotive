 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHAKE_256fSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_256fSimple_utilsx1.c
* \brief  description of SLH_DSA_SHAKE_256fSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void SLH_DSA_SHAKE_256fSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                              const slh_dsa_shake_256f_ctx *const ctx, uint32 leaf_idx,
                                              uint32 idx_offset, uint32 tree_height,
                                              void (*const gen_leaf)(uint8 *const leave /* Where to write the leaves */,
                                                                     const slh_dsa_shake_256f_ctx *const ctx /* ctx */,
                                                                     uint32 idx, void *const info),
                                              uint32 tree_addr[8], void *const info)
{
  /* This is where we keep the intermediate nodes */
  uint8 stack[SLH_DSA_SHAKE_256FSIMPLE_TREEHASHX1_BUF_LEN * SLH_DSA_SHAKE_256FSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < SLH_DSA_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[SLH_DSA_SHAKE_256FSIMPLE_N]. We do this to minimize the number of
     * copies needed during a SLH_DSA_SHAKE_256fSimple_Thash */
    uint8 current[2u * SLH_DSA_SHAKE_256FSIMPLE_N];
    gen_leaf(&current[SLH_DSA_SHAKE_256FSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < SLH_DSA_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        SLH_DSA_CommonLib_MemCpy(root, &current[SLH_DSA_SHAKE_256FSIMPLE_N], SLH_DSA_SHAKE_256FSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        SLH_DSA_CommonLib_MemCpy(&auth_path[h * SLH_DSA_SHAKE_256FSIMPLE_N],
                               &current[SLH_DSA_SHAKE_256FSIMPLE_N], SLH_DSA_SHAKE_256FSIMPLE_N);
      }
      ...
      uint8 *const left = &stack[h * SLH_DSA_SHAKE_256FSIMPLE_N];
      SLH_DSA_CommonLib_MemCpy(&current[0], left, SLH_DSA_SHAKE_256FSIMPLE_N);
      SLH_DSA_SHAKE_256fSimple_Thash(&current[SLH_DSA_SHAKE_256FSIMPLE_N],
                                          &current[0u * SLH_DSA_SHAKE_256FSIMPLE_N], 2u, ctx, tree_addr);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      SLH_DSA_CommonLib_MemCpy(&stack[h * SLH_DSA_SHAKE_256FSIMPLE_N], &current[SLH_DSA_SHAKE_256FSIMPLE_N],
                             SLH_DSA_SHAKE_256FSIMPLE_N);
    }
  }
} // end: SLH_DSA_SHAKE_256fSimple_TreeHashX1
