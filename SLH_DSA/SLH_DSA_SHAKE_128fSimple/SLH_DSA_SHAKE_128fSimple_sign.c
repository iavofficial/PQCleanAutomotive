...
/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_128fSimple_sign.c
* \brief  description of SLH_DSA_SHAKE_128fSimple_sign.c
*
* \details
*
*
...
#include "SLH_DSA_SHAKE_128fSimple_merkle.h"
...
#include "SLH_DSA_SHAKE_128fSimple_wots.h"
#include "SLH_DSA_SHAKE_address.h"

#include "SLH_DSA_SHAKE_128fSimple_sign.h"
/**********************************************************************************************************************/
/* DEFINES                                                                                                            */
/**********************************************************************************************************************/
...
/**********************************************************************************************************************/
static void slh_dsa_shake_128fsimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                  ...
                                                                  const uint8 *const seed)
{
  ...
  SLH_DSA_SHAKE_128fSimple_InitializeHashFunction(&ctx);

  /* Compute root node of the top-most subtree. */
  SLH_DSA_SHAKE_128fSimple_Merkle_GenRoot(&sk[3u * SLH_DSA_SHAKE_128FSIMPLE_N], &ctx);
  ...
  return;
} // end: slh_dsa_shake_128fsimple_crypto_sign_SeedKeyPair
/**********************************************************************************************************************/
...
void SLH_DSA_SHAKE_128fSimple_Crypto_Sign_KeyPair(uint8 *const pk, uint8 *const sk)
{
  ...
  return;
} // end: SLH_DSA_SHAKE_128fSimple_Crypto_Sign_KeyPair
...
designed for use by other systems that aim to integrate the SLH_DSA." */
void SLH_DSA_SHAKE_128fSimple_Crypto_Sign_Signature(uint8 *const sig, uint32 *const siglen, const uint8 *const m,
                                                         uint32 mlen, const uint8 *const sk)
{
  slh_dsa_shake_128f_ctx ctx = {{0}};

  const uint8 *const sk_prf = &sk[SLH_DSA_SHAKE_128FSIMPLE_N];
  const uint8 *const pk     = &sk[2u * SLH_DSA_SHAKE_128FSIMPLE_N];

  uint8 optrand[SLH_DSA_SHAKE_128FSIMPLE_N]            = {0};
  uint8 mhash[SLH_DSA_SHAKE_128FSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[SLH_DSA_SHAKE_128FSIMPLE_N]               = {0};
  uint32 i                                                  = 0;
  uint64 tree                                               = 0;
  uint32 idx_leaf                                           = 0;
  ...
  SLH_DSA_SHAKE_128fSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_N];

  SLH_DSA_SHAKE_SetTreeAddr(wots_addr, tree);
  SLH_DSA_SHAKE_128fSimple_set_keypair_addr(wots_addr, idx_leaf);

  /* Sign the message hash using FORS. */
  SLH_DSA_SHAKE_128fSimple_Fors_Sign(sig_temp, root, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_FORS_BYTES];

  for (i = 0; i < SLH_DSA_SHAKE_128FSIMPLE_D; i++)
  {
    ...
    SLH_DSA_SHAKE_128fSimple_Merkle_Sign(sig_temp, root, &ctx, wots_addr, tree_addr, idx_leaf);
    sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_WOTS_BYTES +
                         (SLH_DSA_SHAKE_128FSIMPLE_TREE_HEIGHT * SLH_DSA_SHAKE_128FSIMPLE_N)];

    /* Update the indices for the next layer. */
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << SLH_DSA_SHAKE_128FSIMPLE_TREE_HEIGHT)) - 1u));
    tree     = tree >> SLH_DSA_SHAKE_128FSIMPLE_TREE_HEIGHT;
  }

  SLH_DSA_SHAKE_128fSimple_1FreeHashFunction(&ctx);

  *siglen = SLH_DSA_SHAKE_128FSIMPLE_BYTES;

  return;
} // end: SLH_DSA_SHAKE_128fSimple_Crypto_Sign_Signature
...
/* polyspace +2 MISRA2012:8.7 [Justified:]"This is an interface function
designed for use by other systems that aim to integrate the SLH_DSA." */
uint8 SLH_DSA_SHAKE_128fSimple_Crypto_Sign_Verify(const uint8 *const sig, uint32 siglen, const uint8 *const m,
                                                       uint32 mlen, const uint8 *const pk)
{
  slh_dsa_shake_128f_ctx ctx                                = {{0}};
  const uint8 *const pub_root                               = &pk[SLH_DSA_SHAKE_128FSIMPLE_N];
  uint8 mhash[SLH_DSA_SHAKE_128FSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[SLH_DSA_SHAKE_128FSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[SLH_DSA_SHAKE_128FSIMPLE_N]               = {0};
  ...
  if (siglen != SLH_DSA_SHAKE_128FSIMPLE_BYTES)
  {
    retVal = ERR_NOT_OK;
  }
   ...
   * The additional SLH_DSA_SHAKE_128FSIMPLE_N is a result of the hash domain separator. */
  SLH_DSA_SHAKE_128fSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_N];
  ...
  SLH_DSA_SHAKE_128fSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < SLH_DSA_SHAKE_128FSIMPLE_D; i++)
  {
    ...
    SLH_DSA_SHAKE_128fSimple_copy_keypair_addr(wots_pk_addr, wots_addr);
    ...
    SLH_DSA_SHAKE_128fSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_WOTS_BYTES];

    /* Compute the leaf node using the WOTS public key. */
    SLH_DSA_SHAKE_128fSimple_Thash(leaf, wots_pk, SLH_DSA_SHAKE_128FSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

    /* Compute the root node of this subtree. */
    SLH_DSA_SHAKE_128fSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
                                              SLH_DSA_SHAKE_128FSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
    sig_temp = &sig_temp[SLH_DSA_SHAKE_128FSIMPLE_TREE_HEIGHT * SLH_DSA_SHAKE_128FSIMPLE_N];
  ...
  return retVal;
} // end: SLH_DSA_SHAKE_128fSimple_Crypto_Sign_Verify
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void SLH_DSA_SHAKE_128fSimple_Crypto_Sign(uint8 *const sm, uint32 *const smlen, const uint8 *const m, uint32 mlen,
                                               const uint8 *const sk)
{
  ...
  *smlen = siglen + mlen;
  ...
  return;
} // end: SLH_DSA_SHAKE_128fSimple_Crypto_Sign
...
/* polyspace +2 MISRA2012:5.1 [Justified:]"The identifiers are distinct. The naming convention ensures clarity
and avoids confusion with other functions. Therefore, this warning is a false positive." */
uint8 SLH_DSA_SHAKE_128fSimple_Crypto_Sign_Open(uint8 *const m, uint32 *const mlen, const uint8 *const sm,
                                                     uint32 smlen, const uint8 *const pk)
{
  ...
  {
    *mlen = smlen - SLH_DSA_SHAKE_128FSIMPLE_BYTES;
