 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHAKE_128fSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_128fSimple_utilsx1.c
* \brief  description of SLH_DSA_SHAKE_128fSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void SLH_DSA_SHAKE_128fSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                              const slh_dsa_shake_128f_ctx *const ctx, uint32 leaf_idx,
                                              ...
                                              uint32 tree_addr[8], void *const info)
{
  ...
  uint8 stack[SLH_DSA_SHAKE_128FSIMPLE_TREEHASHX1_BUF_LEN * SLH_DSA_SHAKE_128FSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < SLH_DSA_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[SLH_DSA_SHAKE_128FSIMPLE_N]. We do this to minimize the number of
     * copies needed during a SLH_DSA_SHAKE_128fSimple_Thash */
    uint8 current[2u * SLH_DSA_SHAKE_128FSIMPLE_N];
    gen_leaf(&current[SLH_DSA_SHAKE_128FSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < SLH_DSA_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        SLH_DSA_CommonLib_MemCpy(root, &current[SLH_DSA_SHAKE_128FSIMPLE_N], SLH_DSA_SHAKE_128FSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        SLH_DSA_CommonLib_MemCpy(&auth_path[h * SLH_DSA_SHAKE_128FSIMPLE_N],
                               &current[SLH_DSA_SHAKE_128FSIMPLE_N], SLH_DSA_SHAKE_128FSIMPLE_N);
      }
      ...
      uint8 *const left = &stack[h * SLH_DSA_SHAKE_128FSIMPLE_N];
      SLH_DSA_CommonLib_MemCpy(&current[0], left, SLH_DSA_SHAKE_128FSIMPLE_N);
      SLH_DSA_SHAKE_128fSimple_Thash(&current[SLH_DSA_SHAKE_128FSIMPLE_N],
                                          &current[0u * SLH_DSA_SHAKE_128FSIMPLE_N], 2u, ctx, tree_addr);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      SLH_DSA_CommonLib_MemCpy(&stack[h * SLH_DSA_SHAKE_128FSIMPLE_N], &current[SLH_DSA_SHAKE_128FSIMPLE_N],
                             SLH_DSA_SHAKE_128FSIMPLE_N);
    }
  }
} // end: SLH_DSA_SHAKE_128fSimple_TreeHashX1
