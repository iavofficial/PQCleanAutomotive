 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHA2_128sSimple_sign
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHA2_128sSimple_sign.c
* \brief  description of SLH_DSA_SHA2_128sSimple_sign.c
*
* \details
*
*
...
#include "SLH_DSA_SHA2_128sSimple_merkle.h"
#include "SLH_DSA_SHA2_128sSimple_params.h"
#include "SLH_DSA_SHA2_128sSimple_thash.h"
#include "SLH_DSA_SHA2_128sSimple_utils.h"
#include "SLH_DSA_SHA2_128sSimple_wots.h"
#include "SLH_DSA_SHA2_address.h"

#include "SLH_DSA_SHA2_128sSimple_sign.h"
/**********************************************************************************************************************/
/* GLOBAL DEFINES                                                                                                     */
/**********************************************************************************************************************/
...
/**********************************************************************************************************************/
static void slh_dsa_sha2_128ssimple_crypto_sign_SeedKeyPair(uint8 *const pk, uint8 *const sk,
                                                                 ...
                                                                 const uint8 *const seed)
{
  ...
  SLH_DSA_SHA2_128sSimple_InitializeHashFunction(&ctx);
  ...
  return;
} // end: slh_dsa_sha2_128ssimple_crypto_sign_SeedKeyPair
/**********************************************************************************************************************/
...
void SLH_DSA_SHA2_128sSimple_Crypto_Sign_KeyPair(uint8 *const pk, uint8 *const sk)
{
  ...
  return;
} // end: SLH_DSA_SHA2_128sSimple_Crypto_Sign_KeyPair
...
designed for use by other systems that aim to integrate the SLH_DSA." */
void SLH_DSA_SHA2_128sSimple_Crypto_Sign_Signature(uint8 *const sig, uint32 *const siglen, const uint8 *const m,
                                                        uint32 mlen, const uint8 *const sk)
{
  slh_dsa_sha2_128s_ctx ctx = {{0}};

  const uint8 *const sk_prf = &sk[SLH_DSA_SHA2_128SSIMPLE_N];
  const uint8 *const pk     = &sk[2u * SLH_DSA_SHA2_128SSIMPLE_N];

  uint8 optrand[SLH_DSA_SHA2_128SSIMPLE_N]            = {0};
  uint8 mhash[SLH_DSA_SHA2_128SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 root[SLH_DSA_SHA2_128SSIMPLE_N]               = {0};
  uint32 i                                                 = 0;
  uint64 tree                                              = 0;
  uint32 idx_leaf                                          = 0;
  ...
  SLH_DSA_SHA2_128sSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_N];

  SLH_DSA_SHA2_SetTreeAddr(wots_addr, tree);
  SLH_DSA_SHA2_128sSimple_set_keypair_addr(wots_addr, idx_leaf);

  /* Sign the message hash using FORS. */
  SLH_DSA_SHA2_128sSimple_Fors_Sign(sig_temp, root, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_FORS_BYTES];

  for (i = 0; i < SLH_DSA_SHA2_128SSIMPLE_D; i++)
  {
    ...
    SLH_DSA_SHA2_128sSimple_Merkle_Sign(sig_temp, root, &ctx, wots_addr, tree_addr, idx_leaf);
    sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_WOTS_BYTES +
                         (SLH_DSA_SHA2_128SSIMPLE_TREE_HEIGHT * SLH_DSA_SHA2_128SSIMPLE_N)];

    /* Update the indices for the next layer. */
    idx_leaf = (uint32)(tree & (((uint64)((uint64)1u << SLH_DSA_SHA2_128SSIMPLE_TREE_HEIGHT)) - 1u));
    tree     = tree >> SLH_DSA_SHA2_128SSIMPLE_TREE_HEIGHT;
  }

  *siglen = SLH_DSA_SHA2_128SSIMPLE_BYTES;

  return;
} // end: SLH_DSA_SHA2_128sSimple_Crypto_Sign_Signature
...
/* polyspace +2 MISRA2012:8.7 [Justified:]"This is an interface function
designed for use by other systems that aim to integrate the SLH_DSA." */
uint8 SLH_DSA_SHA2_128sSimple_Crypto_Sign_Verify(const uint8 *const sig, uint32 siglen, const uint8 *const m,
                                                      uint32 mlen, const uint8 *const pk)
{
  slh_dsa_sha2_128s_ctx ctx                                = {{0}};
  const uint8 *const pub_root                              = &pk[SLH_DSA_SHA2_128SSIMPLE_N];
  uint8 mhash[SLH_DSA_SHA2_128SSIMPLE_FORS_MSG_BYTES] = {0};
  uint8 wots_pk[SLH_DSA_SHA2_128SSIMPLE_WOTS_BYTES]   = {0};
  uint8 root[SLH_DSA_SHA2_128SSIMPLE_N]               = {0};
  uint8 leaf[SLH_DSA_SHA2_128SSIMPLE_N]               = {0};
  uint32 i                                                 = 0;
  uint64 tree                                              = 0;
  uint32 idx_leaf                                          = 0;
  uint32 wots_addr[SLH_DSA_SIGN_ADDR_SIZE]           = {0};
  ...
  if (siglen != SLH_DSA_SHA2_128SSIMPLE_BYTES)
  {
    retVal = ERR_NOT_OK;
  }
  ...
  /* The additional SLH_DSA_SHA2_128SSIMPLE_N is a result of the hash domain separator. */
  SLH_DSA_SHA2_128sSimple_HashMessage(mhash, &tree, &idx_leaf, sig_temp, pk, m, mlen, &ctx);
  sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_N];
  ...
  SLH_DSA_SHA2_128sSimple_Fors_PkFromSig(root, sig_temp, mhash, &ctx, wots_addr);
  sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_FORS_BYTES];
  ...
  for (i = 0; i < SLH_DSA_SHA2_128SSIMPLE_D; i++)
  {
    ...
    SLH_DSA_SHA2_128sSimple_copy_keypair_addr(wots_pk_addr, wots_addr);

    /* The WOTS public key is only correct if the signature was correct. */
    /* Initially, root is the FORS pk, but on subsequent iterations it is
       the root of the subtree below the currently processed subtree. */
    SLH_DSA_SHA2_128sSimple_Wots_PkFromSig(wots_pk, sig_temp, root, &ctx, wots_addr);
    sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_WOTS_BYTES];

    /* Compute the leaf node using the WOTS public key. */
    SLH_DSA_SHA2_128sSimple_Thash(leaf, wots_pk, SLH_DSA_SHA2_128SSIMPLE_WOTS_LEN, &ctx, wots_pk_addr);

    /* Compute the root node of this subtree. */
    SLH_DSA_SHA2_128sSimple_ComputeRoot(root, leaf, idx_leaf, 0, sig_temp,
                                             SLH_DSA_SHA2_128SSIMPLE_TREE_HEIGHT, &ctx, tree_addr);
    sig_temp = &sig_temp[SLH_DSA_SHA2_128SSIMPLE_TREE_HEIGHT * SLH_DSA_SHA2_128SSIMPLE_N];
  ...
  return retVal;
} // end: SLH_DSA_SHA2_128sSimple_Crypto_Sign_Verify
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void SLH_DSA_SHA2_128sSimple_Crypto_Sign(uint8 *const sm, uint32 *const smlen, const uint8 *const m, uint32 mlen,
                                              const uint8 *const sk)
{
  ...
  *smlen = siglen + mlen;
  ...
  return;
} // end: SLH_DSA_SHA2_128sSimple_Crypto_Sign
...
/* polyspace +2 MISRA2012:5.1 [Justified:]"The identifiers are distinct. The naming convention ensures clarity
and avoids confusion with other functions. Therefore, this warning is a false positive." */
uint8 SLH_DSA_SHA2_128sSimple_Crypto_Sign_Open(uint8 *const m, uint32 *const mlen, const uint8 *const sm,
                                                    uint32 smlen, const uint8 *const pk)
{
  ...
  {
    *mlen = smlen - SLH_DSA_SHA2_128SSIMPLE_BYTES;
