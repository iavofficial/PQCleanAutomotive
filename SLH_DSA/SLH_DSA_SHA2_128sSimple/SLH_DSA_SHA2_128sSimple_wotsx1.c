 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHA2_128sSimple_wotsx1
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHA2_128sSimple_wotsx1.c
* \brief  description of SLH_DSA_SHA2_128sSimple_wotsx1.c
*
* \details
*
*
 ...
 * \returns a1.
 */
void SLH_DSA_SHA2_128sSimple_Wots_GenLeafX1(uint8 *const dest, const slh_dsa_sha2_128s_ctx *const ctx,
                                                 uint32 leaf_idx, void *const v_info)
{
  ...
  /* polyspace +2 MISRA2012:11.5 [Justified:]"Necessary conversion from void* to object* for functionality.
    Ensured proper alignment and validity." */
  SLH_DSA_SHA2_128sSimple_LeafInfoX1_T *info          = v_info;
  uint32 *const leaf_addr                                  = info->leaf_addr;
  uint32 *const pk_addr                                    = info->pk_addr;
  uint32 i                                                 = 0;
  uint32 k                                                 = 0;
  uint8 pk_buffer[SLH_DSA_SHA2_128SSIMPLE_WOTS_BYTES] = {0};
  ...
  SLH_DSA_SHA2_128sSimple_set_keypair_addr(leaf_addr, leaf_idx);
  SLH_DSA_SHA2_128sSimple_set_keypair_addr(pk_addr, leaf_idx);
  ...
  for (i = 0; i < SLH_DSA_SHA2_128SSIMPLE_WOTS_LEN; i++)
  {
    ...
    SLH_DSA_SHA2_128sSimple_PrfAddr(buffer, ctx, leaf_addr);
    ...
    for (k = 0; k < SLH_DSA_UINT32_MAX_VALUE; k++)
    {
      ...
      if (k == wots_k)
      {
        SLH_DSA_CommonLib_MemCpy(&info->wots_sig[i * SLH_DSA_SHA2_128SSIMPLE_N], buffer,
                               SLH_DSA_SHA2_128SSIMPLE_N);
      }

      /* Check if we hit the top of the chain */
      if (k == SLH_DSA_SHA2_128SSIMPLE_WOTS_W - 1u)
      {
        break;
      }
      ...
      SLH_DSA_SHA2_128sSimple_Thash(buffer, buffer, 1, ctx, leaf_addr);
    }

    buffer = &buffer[SLH_DSA_SHA2_128SSIMPLE_N];
  }

  /* Do the final thash to generate the public keys */
  SLH_DSA_SHA2_128sSimple_Thash(dest, pk_buffer, SLH_DSA_SHA2_128SSIMPLE_WOTS_LEN, ctx, pk_addr);
} // end: SLH_DSA_SHA2_128sSimple_Wots_GenLeafX1
