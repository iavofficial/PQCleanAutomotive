 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHA2_128sSimple_utilsx1
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHA2_128sSimple_utilsx1.c
* \brief  description of SLH_DSA_SHA2_128sSimple_utilsx1.c
*
* \details
*
*
...
and avoids confusion with other functions. Therefore, this warning is a false positive." */
void SLH_DSA_SHA2_128sSimple_TreeHashX1(uint8 *const root, uint8 *const auth_path,
                                             ...
                                             void (*const gen_leaf)(uint8 *const out /* Where to write the leaves */,
                                                                    const slh_dsa_sha2_128s_ctx *const ctx, uint32 idx,
                                                                    void *const info),
                                             uint32 tree_addr[8], void *const info)
{
  uint8 stack[SLH_DSA_SHA2_128SSIMPLE_TREEHASHX1_BUF_LEN * SLH_DSA_SHA2_128SSIMPLE_N] = {0};
  ...
  for (idx = 0; idx < SLH_DSA_UINT32_MAX_VALUE; idx++)
  {
    ...
    /* Current logical node is at index[SLH_DSA_SHA2_128SSIMPLE_N]. We do this to minimize the number of copies
     * needed during a SLH_DSA_SHA2_128sSimple_Thash */
    uint8 current[2u * SLH_DSA_SHA2_128SSIMPLE_N];
    gen_leaf(&current[SLH_DSA_SHA2_128SSIMPLE_N], ctx, idx + idx_offset, info);
    ...
    for (h = 0; h < SLH_DSA_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        SLH_DSA_CommonLib_MemCpy(root, &current[SLH_DSA_SHA2_128SSIMPLE_N], SLH_DSA_SHA2_128SSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        SLH_DSA_CommonLib_MemCpy(&auth_path[h * SLH_DSA_SHA2_128SSIMPLE_N], &current[SLH_DSA_SHA2_128SSIMPLE_N],
      ...
      SLH_DSA_SHA2_SetTreeHeight(tree_addr, h + 1u);
      SLH_DSA_SHA2_SetTreeIndex(tree_addr, (internal_idx / 2u) + internal_idx_offset);

      uint8 *const left = &stack[h * SLH_DSA_SHA2_128SSIMPLE_N];
      SLH_DSA_CommonLib_MemCpy(&current[0], left, SLH_DSA_SHA2_128SSIMPLE_N);
      SLH_DSA_SHA2_128sSimple_Thash(&current[SLH_DSA_SHA2_128SSIMPLE_N],
                                         &current[0u * SLH_DSA_SHA2_128SSIMPLE_N], 2u, ctx, tree_addr);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      SLH_DSA_CommonLib_MemCpy(&stack[h * SLH_DSA_SHA2_128SSIMPLE_N], &current[SLH_DSA_SHA2_128SSIMPLE_N],
                             SLH_DSA_SHA2_128SSIMPLE_N);
    }
  }
} // end: SLH_DSA_SHA2_128sSimple_TreeHashX1
