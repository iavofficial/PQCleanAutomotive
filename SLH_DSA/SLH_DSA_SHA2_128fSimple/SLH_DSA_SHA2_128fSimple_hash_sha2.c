                                          ...
                                          const uint32 addr[8])
{
  ...
  FsmSw_CommonLib_MemCpy(&buf[SPX_SHA256_ADDR_BYTES], ctx->sk_seed, SLH_DSA_SHA2_128FSIMPLE_N);

  FsmSw_Sha256_IncFinalize(outbuf, &sha2_state, buf, SPX_SHA256_ADDR_BYTES + SLH_DSA_SHA2_128FSIMPLE_N);

  FsmSw_CommonLib_MemCpy(out, outbuf, SLH_DSA_SHA2_128FSIMPLE_N);
                                                   ...
                                                   const slh_dsa_sha2_128f_ctx *const ctx)
{
  ...
  FsmSw_CommonLib_MemSet(&buf[SLH_DSA_SHA2_128FSIMPLE_N], 0x36u,
                         SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N);

  shaX_inc_init(&state);
  shaX_inc_blocks(&state, buf, 1u);

  FsmSw_CommonLib_MemCpy(buf, optrand, SLH_DSA_SHA2_128FSIMPLE_N);

  /* If optrand + message cannot fill up an entire block */
  if ((SLH_DSA_SHA2_128FSIMPLE_N + mlen_temp) < SPX_SHAX_BLOCK_BYTES)
  {
    FsmSw_CommonLib_MemCpy(&buf[SLH_DSA_SHA2_128FSIMPLE_N], m_temp, mlen_temp);
    shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state, buf, mlen_temp + SLH_DSA_SHA2_128FSIMPLE_N);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    FsmSw_CommonLib_MemCpy(&buf[SLH_DSA_SHA2_128FSIMPLE_N], m_temp,
                           SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N);
    shaX_inc_blocks(&state, buf, 1u);
  ...
  FsmSw_CommonLib_MemSet(&buf[SLH_DSA_SHA2_128FSIMPLE_N], 0x5Cu,
                         SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N);

  shaX(buf, buf, SPX_SHAX_BLOCK_BYTES + SPX_SHAX_OUTPUT_BYTES);
  FsmSw_CommonLib_MemCpy(R, buf, SLH_DSA_SHA2_128FSIMPLE_N);
                                              ...
                                              uint32 mlen, const slh_dsa_sha2_128f_ctx *const ctx)
{
  ...
  FsmSw_CommonLib_MemCpy(inbuf, R, SLH_DSA_SHA2_128FSIMPLE_N);
  FsmSw_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_128FSIMPLE_N], pk, SLH_DSA_SHA2_128FSIMPLE_PK_BYTES);
  ...
  {
    FsmSw_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_128FSIMPLE_N + SLH_DSA_SHA2_128FSIMPLE_PK_BYTES], m_temp,
                           mlen_temp);
  ...
  FsmSw_CommonLib_MemCpy(seed, R, SLH_DSA_SHA2_128FSIMPLE_N);
  FsmSw_CommonLib_MemCpy(&seed[SLH_DSA_SHA2_128FSIMPLE_N], pk, SLH_DSA_SHA2_128FSIMPLE_N);

  /* By doing this in two steps, we prevent hashing the message twice;
     otherwise each iteration in MGF1 would hash the message again. */
  mgf1_X(bufp, SPX_DGST_BYTES, seed, (2u * SLH_DSA_SHA2_128FSIMPLE_N) + SPX_SHAX_OUTPUT_BYTES);

  FsmSw_CommonLib_MemCpy(digest, bufp, SLH_DSA_SHA2_128FSIMPLE_FORS_MSG_BYTES);
