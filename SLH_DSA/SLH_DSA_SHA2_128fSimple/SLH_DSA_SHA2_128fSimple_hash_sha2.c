...
/*====================================================================================================================*/
/** \file SLH_DSA_SHA2_128fSimple_hash_sha2.c
* \brief  description of SLH_DSA_SHA2_128fSimple_hash_sha2.c
*
* \details
*
*
...
#define mgf1_X                SLH_DSA_SHA2_128fSimple_MgF1_256
#define shaXstate             sha256ctx

#define SPX_TREE_BITS  (SLH_DSA_SHA2_128FSIMPLE_TREE_HEIGHT * (SLH_DSA_SHA2_128FSIMPLE_D - 1u))
#define SPX_TREE_BYTES ((SPX_TREE_BITS + 7u) / 8u)
#define SPX_LEAF_BITS  SLH_DSA_SHA2_128FSIMPLE_TREE_HEIGHT
#define SPX_LEAF_BYTES ((SPX_LEAF_BITS + 7u) / 8u)
#define SPX_DGST_BYTES (SLH_DSA_SHA2_128FSIMPLE_FORS_MSG_BYTES + SPX_TREE_BYTES + SPX_LEAF_BYTES)
...
#define SPX_INBLOCKS                                                                                                   \
  (uint32)((uint32)((uint32)(((SLH_DSA_SHA2_128FSIMPLE_N + SLH_DSA_SHA2_128FSIMPLE_PK_BYTES +                \
...
/*====================================================================================================================*/
/**
 * \brief SLH_DSA_SHA2_128fSimple_MgF1_256 function based on the SHA-256 hash function. Note that inlen
...
{
  uint8 inbuf[SLH_DSA_SHA2_128FSIMPLE_MGF1_256_BUF_LEN + 4u] = {0};
...
} // end: SLH_DSA_SHA2_128fSimple_MgF1_256
                                          ...
                                          const uint32 addr[8])
{
  ...
  uint8 buf[SPX_SHA256_ADDR_BYTES + SLH_DSA_SHA2_128FSIMPLE_N] = {0};
  uint8 outbuf[SLH_DSA_SHA256_OUTPUT_BYTES]                   = {0};
  ...
  SLH_DSA_CommonLib_MemCpy(&buf[SPX_SHA256_ADDR_BYTES], ctx->sk_seed, SLH_DSA_SHA2_128FSIMPLE_N);

  SLH_DSA_Sha256_IncFinalize(outbuf, &sha2_state, buf, SPX_SHA256_ADDR_BYTES + SLH_DSA_SHA2_128FSIMPLE_N);

  SLH_DSA_CommonLib_MemCpy(out, outbuf, SLH_DSA_SHA2_128FSIMPLE_N);
} // end: SLH_DSA_SHA2_128fSimple_PrfAddr
 ...
 *        SLH_DSA_SHA2_128FSIMPLE_N space available in front of the pointer, i.e. before the message to use
 *        for the prefix.
                                                   ...
                                                   const slh_dsa_sha2_128f_ctx *const ctx)
{
  ...
  SLH_DSA_CommonLib_MemSet(&buf[SLH_DSA_SHA2_128FSIMPLE_N], 0x36u,
                         SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N);

  shaX_inc_init(&state);
  shaX_inc_blocks(&state, buf, 1u);

  SLH_DSA_CommonLib_MemCpy(buf, optrand, SLH_DSA_SHA2_128FSIMPLE_N);

  /* If optrand + message cannot fill up an entire block */
  if ((SLH_DSA_SHA2_128FSIMPLE_N + mlen_temp) < SPX_SHAX_BLOCK_BYTES)
  {
    SLH_DSA_CommonLib_MemCpy(&buf[SLH_DSA_SHA2_128FSIMPLE_N], m_temp, mlen_temp);
    shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state, buf, mlen_temp + SLH_DSA_SHA2_128FSIMPLE_N);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    SLH_DSA_CommonLib_MemCpy(&buf[SLH_DSA_SHA2_128FSIMPLE_N], m_temp,
                           SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N);
    shaX_inc_blocks(&state, buf, 1u);

    m_temp = &m_temp[SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N];
    mlen_temp -= SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N;
  ...
  SLH_DSA_CommonLib_MemSet(&buf[SLH_DSA_SHA2_128FSIMPLE_N], 0x5Cu,
                         SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_128FSIMPLE_N);

  shaX(buf, buf, SPX_SHAX_BLOCK_BYTES + SPX_SHAX_OUTPUT_BYTES);
  SLH_DSA_CommonLib_MemCpy(R, buf, SLH_DSA_SHA2_128FSIMPLE_N);
} // end: SLH_DSA_SHA2_128fSimple_GenMessageRandom
...
{
  (void)ctx;
  uint8 seed[(2u * SLH_DSA_SHA2_128FSIMPLE_N) + SPX_SHAX_OUTPUT_BYTES] = {0};
  ...
  SLH_DSA_CommonLib_MemCpy(inbuf, R, SLH_DSA_SHA2_128FSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_128FSIMPLE_N], pk, SLH_DSA_SHA2_128FSIMPLE_PK_BYTES);

  /* If R + pk + message cannot fill up an entire block */
  /* polyspace +3 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
  and therefore not dead code" */
  if ((SLH_DSA_SHA2_128FSIMPLE_N + SLH_DSA_SHA2_128FSIMPLE_PK_BYTES + mlen_temp) <
      (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES))
  {
    SLH_DSA_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_128FSIMPLE_N + SLH_DSA_SHA2_128FSIMPLE_PK_BYTES], m_temp,
                           mlen_temp);
    shaX_inc_finalize(&seed[2u * SLH_DSA_SHA2_128FSIMPLE_N], &state, inbuf,
                      SLH_DSA_SHA2_128FSIMPLE_N + SLH_DSA_SHA2_128FSIMPLE_PK_BYTES + mlen_temp);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    ...
    and therefore not dead code" */
    SLH_DSA_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_128FSIMPLE_N + SLH_DSA_SHA2_128FSIMPLE_PK_BYTES], m_temp,
                           (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - SLH_DSA_SHA2_128FSIMPLE_N -
                               SLH_DSA_SHA2_128FSIMPLE_PK_BYTES);
    shaX_inc_blocks(&state, inbuf, SPX_INBLOCKS);

    /* polyspace +2 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
    and therefore not dead code" */
    m_temp = &m_temp[(SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - SLH_DSA_SHA2_128FSIMPLE_N -
                     SLH_DSA_SHA2_128FSIMPLE_PK_BYTES];
    /* polyspace +3 MISRA2012:2.2 [Justified:]"Calculation is important if defines should change
    and therefore not dead code" */
    mlen_temp -=
        (SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) - SLH_DSA_SHA2_128FSIMPLE_N - SLH_DSA_SHA2_128FSIMPLE_PK_BYTES;
    shaX_inc_finalize(&seed[2u * SLH_DSA_SHA2_128FSIMPLE_N], &state, m_temp, (uint32)mlen_temp);
  }

  /* H_msg: MGF1-SHA-X(R ‖ PK.seed ‖ seed) */
  SLH_DSA_CommonLib_MemCpy(seed, R, SLH_DSA_SHA2_128FSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(&seed[SLH_DSA_SHA2_128FSIMPLE_N], pk, SLH_DSA_SHA2_128FSIMPLE_N);

  /* By doing this in two steps, we prevent hashing the message twice;
     otherwise each iteration in MGF1 would hash the message again. */
  mgf1_X(bufp, SPX_DGST_BYTES, seed, (2u * SLH_DSA_SHA2_128FSIMPLE_N) + SPX_SHAX_OUTPUT_BYTES);

  SLH_DSA_CommonLib_MemCpy(digest, bufp, SLH_DSA_SHA2_128FSIMPLE_FORS_MSG_BYTES);
  bufp = &bufp[SLH_DSA_SHA2_128FSIMPLE_FORS_MSG_BYTES];
...
} // end: SLH_DSA_SHA2_128fSimple_HashMessage
