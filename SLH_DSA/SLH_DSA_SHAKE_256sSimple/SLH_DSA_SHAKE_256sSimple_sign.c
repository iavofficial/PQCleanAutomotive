                                                                  ...
                                                                  const uint8 *const seed)
{
  ...
  SLH_DSA_CommonLib_MemCpy(sk, seed, SLH_DSA_SHAKE_256SSIMPLE_CRYPTO_SEEDBYTES);

  SLH_DSA_CommonLib_MemCpy(pk, &sk[2u * SLH_DSA_SHAKE_256SSIMPLE_N], SLH_DSA_SHAKE_256SSIMPLE_N);

  SLH_DSA_CommonLib_MemCpy(ctx.pub_seed, pk, SLH_DSA_SHAKE_256SSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(ctx.sk_seed, sk, SLH_DSA_SHAKE_256SSIMPLE_N);
  ...
  SLH_DSA_CommonLib_MemCpy(&pk[SLH_DSA_SHAKE_256SSIMPLE_N], &sk[3u * SLH_DSA_SHAKE_256SSIMPLE_N],
...
void SLH_DSA_SHAKE_256sSimple_Crypto_Sign_KeyPair(uint8 *const pk, uint8 *const sk)
{
  ...
  (void)SLH_DSA_CommonLib_RandomBytes(seed, SLH_DSA_SHAKE_256SSIMPLE_CRYPTO_SEEDBYTES);
                                                         ...
                                                         uint32 mlen, const uint8 *const sk)
{
  ...
  (void)SLH_DSA_CommonLib_RandomBytes(optrand, SLH_DSA_SHAKE_256SSIMPLE_N);
                                                       ...
                                                       uint32 mlen, const uint8 *const pk)
{
  ...
  if (SLH_DSA_CommonLib_MemCmp(root, pub_root, SLH_DSA_SHAKE_256SSIMPLE_N) != 0u)
  {
    retVal = ERR_NOT_OK;
  }
                                               ...
                                               const uint8 *const sk)
{
  ...
  SLH_DSA_CommonLib_MemMove(&sm[SLH_DSA_SHAKE_256SSIMPLE_BYTES], m, mlen);
  *smlen = siglen + mlen;
                                                     ...
                                                     uint32 smlen, const uint8 *const pk)
{
  ...
  else
  {
    *mlen = smlen - SLH_DSA_SHAKE_256SSIMPLE_BYTES;
    ...
    SLH_DSA_CommonLib_MemMove(m, &sm[SLH_DSA_SHAKE_256SSIMPLE_BYTES], *mlen);
  }
  return retVal;
