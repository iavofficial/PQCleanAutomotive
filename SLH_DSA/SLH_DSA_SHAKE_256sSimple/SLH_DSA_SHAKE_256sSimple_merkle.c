...
/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_256sSimple_merkle.c
* \brief  description of SLH_DSA_SHAKE_256sSimple_merkle.c
*
* \details
*
*
...
{
  uint8 *const auth_path = &sig[SLH_DSA_SHAKE_256SSIMPLE_WOTS_BYTES];

  SLH_DSA_SHAKE_256sSimple_LeafInfoX1_T info = {
      ((void *)0),
  ...
  uint32 steps[SLH_DSA_SHAKE_256SSIMPLE_WOTS_LEN] = {0};

  info.wots_sig = sig;
  SLH_DSA_SHAKE_256sSimple_Wots_ChainLengths(steps, root);
  ...
  SLH_DSA_SHAKE_256sSimple_TreeHashX1(root, auth_path, ctx, idx_leaf, 0, SLH_DSA_SHAKE_256SSIMPLE_TREE_HEIGHT,
                                           SLH_DSA_SHAKE_256sSimple_Wots_GenLeafX1, tree_addr, &info);
} // end: SLH_DSA_SHAKE_256sSimple_Merkle_Sign
...
void SLH_DSA_SHAKE_256sSimple_Merkle_GenRoot(uint8 *const root, const slh_dsa_shake_256s_ctx *const ctx)
{
   ...
   * SLH_DSA_SHAKE_256sSimple_TreeHash routine that computes both root and path in one function. */
  uint8 auth_path[(SLH_DSA_SHAKE_256SSIMPLE_TREE_HEIGHT * SLH_DSA_SHAKE_256SSIMPLE_N) +
                  SLH_DSA_SHAKE_256SSIMPLE_WOTS_BYTES] = {0};
  uint32 top_tree_addr[SLH_DSA_SIGN_ADDR_SIZE]        = {0};
  uint32 wots_addr[SLH_DSA_SIGN_ADDR_SIZE]            = {0};

  SLH_DSA_SHAKE_SetLayerAddr(top_tree_addr, SLH_DSA_SHAKE_256SSIMPLE_D - 1u);
  SLH_DSA_SHAKE_SetLayerAddr(wots_addr, SLH_DSA_SHAKE_256SSIMPLE_D - 1u);

  SLH_DSA_SHAKE_256sSimple_Merkle_Sign(auth_path, root, ctx, wots_addr, top_tree_addr,
                                            ~((uint32)0u) /* ~0 means "don't bother generating an auth path */);
} // end: SLH_DSA_SHAKE_256sSimple_Merkle_GenRoot
