                                          ...
                                          const uint32 addr[8])
{
  ...
  SLH_DSA_CommonLib_MemCpy(&buf[SPX_SHA256_ADDR_BYTES], ctx->sk_seed, SLH_DSA_SHA2_256SSIMPLE_N);

  SLH_DSA_Sha256_IncFinalize(outbuf, &sha2_state, buf, SPX_SHA256_ADDR_BYTES + SLH_DSA_SHA2_256SSIMPLE_N);
                                                   ...
                                                   const slh_dsa_sha2_256s_ctx *const ctx)
{
  ...
  SLH_DSA_CommonLib_MemSet(&buf[SLH_DSA_SHA2_256SSIMPLE_N], 0x36u,
                         SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_256SSIMPLE_N);

  shaX_inc_init(&state);
  shaX_inc_blocks(&state, buf, 1u);

  SLH_DSA_CommonLib_MemCpy(buf, optrand, SLH_DSA_SHA2_256SSIMPLE_N);

  /* If optrand + message cannot fill up an entire block */
  if ((SLH_DSA_SHA2_256SSIMPLE_N + mlen_temp) < SPX_SHAX_BLOCK_BYTES)
  {
    SLH_DSA_CommonLib_MemCpy(&buf[SLH_DSA_SHA2_256SSIMPLE_N], m_temp, mlen_temp);
    shaX_inc_finalize(&buf[SPX_SHAX_BLOCK_BYTES], &state, buf, mlen_temp + SLH_DSA_SHA2_256SSIMPLE_N);
  }
  /* Otherwise first fill a block, so that finalize only uses the message */
  else
  {
    SLH_DSA_CommonLib_MemCpy(&buf[SLH_DSA_SHA2_256SSIMPLE_N], m_temp,
                           SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_256SSIMPLE_N);
    shaX_inc_blocks(&state, buf, 1u);
  ...
  SLH_DSA_CommonLib_MemSet(&buf[SLH_DSA_SHA2_256SSIMPLE_N], 0x5Cu,
                         SPX_SHAX_BLOCK_BYTES - SLH_DSA_SHA2_256SSIMPLE_N);

  shaX(buf, buf, SPX_SHAX_BLOCK_BYTES + SPX_SHAX_OUTPUT_BYTES);
  SLH_DSA_CommonLib_MemCpy(R, buf, SLH_DSA_SHA2_256SSIMPLE_N);
                                              ...
                                              uint32 mlen, const slh_dsa_sha2_256s_ctx *const ctx)
{
  ...
  SLH_DSA_CommonLib_MemCpy(inbuf, R, SLH_DSA_SHA2_256SSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_256SSIMPLE_N], pk, SLH_DSA_SHA2_256SSIMPLE_PK_BYTES);
  ...
  {
    SLH_DSA_CommonLib_MemCpy(&inbuf[SLH_DSA_SHA2_256SSIMPLE_N + SLH_DSA_SHA2_256SSIMPLE_PK_BYTES], m_temp,
                           mlen_temp);
  ...
  SLH_DSA_CommonLib_MemCpy(seed, R, SLH_DSA_SHA2_256SSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(&seed[SLH_DSA_SHA2_256SSIMPLE_N], pk, SLH_DSA_SHA2_256SSIMPLE_N);

  /* By doing this in two steps, we prevent hashing the message twice;
     otherwise each iteration in MGF1 would hash the message again. */
  mgf1_X(bufp, SPX_DGST_BYTES, seed, (2u * SLH_DSA_SHA2_256SSIMPLE_N) + SPX_SHAX_OUTPUT_BYTES);

  SLH_DSA_CommonLib_MemCpy(digest, bufp, SLH_DSA_SHA2_256SSIMPLE_FORS_MSG_BYTES);
