 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHAKE_192fSimple_hash
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_192fSimple_hash_shake.c
* \brief  description of SLH_DSA_SHAKE_192fSimple_hash_shake.c
*
* \details
*
*
...
/* DEFINES                                                                                                            */
/**********************************************************************************************************************/
#define SPX_TREE_BITS  (SLH_DSA_SHAKE_192FSIMPLE_TREE_HEIGHT * (SLH_DSA_SHAKE_192FSIMPLE_D - 1u))
#define SPX_TREE_BYTES ((SPX_TREE_BITS + 7u) / 8u)
#define SPX_LEAF_BITS  SLH_DSA_SHAKE_192FSIMPLE_TREE_HEIGHT
#define SPX_LEAF_BYTES ((SPX_LEAF_BITS + 7u) / 8u)
#define SPX_DGST_BYTES (SLH_DSA_SHAKE_192FSIMPLE_FORS_MSG_BYTES + SPX_TREE_BYTES + SPX_LEAF_BYTES)
 ...
 *
 */
void SLH_DSA_SHAKE_192fSimple_PrfAddr(uint8 *const out, const slh_dsa_shake_192f_ctx *const ctx,
                                           const uint32 addr[8])
{
  uint8 buf[(2u * SLH_DSA_SHAKE_192FSIMPLE_N) + SLH_DSA_SHAKE_192FSIMPLE_ADDR_BYTES] = {0};

  SLH_DSA_CommonLib_MemCpy(buf, ctx->pub_seed, SLH_DSA_SHAKE_192FSIMPLE_N);
  SLH_DSA_CommonLib_MemCpy(&buf[SLH_DSA_SHAKE_192FSIMPLE_N], addr, SLH_DSA_SHAKE_192FSIMPLE_ADDR_BYTES);
  SLH_DSA_CommonLib_MemCpy(&buf[SLH_DSA_SHAKE_192FSIMPLE_N + SLH_DSA_SHAKE_192FSIMPLE_ADDR_BYTES], ctx->sk_seed,
                         SLH_DSA_SHAKE_192FSIMPLE_N);

  SLH_DSA_Fips202_Shake256(out, SLH_DSA_SHAKE_192FSIMPLE_N, buf,
                         (2u * SLH_DSA_SHAKE_192FSIMPLE_N) + SLH_DSA_SHAKE_192FSIMPLE_ADDR_BYTES);
} // end: SLH_DSA_SHAKE_192fSimple_PrfAddr
 ...
 *
 */
void SLH_DSA_SHAKE_192fSimple_GenMessageRandom(uint8 *const R, const uint8 *const sk_prf,
                                                    ...
                                                    const slh_dsa_shake_192f_ctx *const ctx)
{
  ...
  SLH_DSA_Fips202_Shake256_IncAbsorb(&s_inc, sk_prf, SLH_DSA_SHAKE_192FSIMPLE_N);
  SLH_DSA_Fips202_Shake256_IncAbsorb(&s_inc, optrand, SLH_DSA_SHAKE_192FSIMPLE_N);
  SLH_DSA_Fips202_Shake256_IncAbsorb(&s_inc, m, mlen);
  SLH_DSA_Fips202_Shake256_IncFinalize(&s_inc);
  SLH_DSA_Fips202_Shake256_IncSqueeze(R, SLH_DSA_SHAKE_192FSIMPLE_N, &s_inc);
} // end: SLH_DSA_SHAKE_192fSimple_GenMessageRandom
 ...
 *
 */
void SLH_DSA_SHAKE_192fSimple_HashMessage(uint8 *const digest, uint64 *const tree, uint32 *const leaf_idx,
                                               const uint8 *const R, const uint8 *const pk, const uint8 *const m,
                                               uint32 mlen, const slh_dsa_shake_192f_ctx *const ctx)
{
  (void)ctx;
  ...
  SLH_DSA_Fips202_Shake256_IncInit(&s_inc);
  SLH_DSA_Fips202_Shake256_IncAbsorb(&s_inc, R, SLH_DSA_SHAKE_192FSIMPLE_N);
  SLH_DSA_Fips202_Shake256_IncAbsorb(&s_inc, pk, SLH_DSA_SHAKE_192FSIMPLE_PK_BYTES);
  SLH_DSA_Fips202_Shake256_IncAbsorb(&s_inc, m, mlen);
  SLH_DSA_Fips202_Shake256_IncFinalize(&s_inc);
  SLH_DSA_Fips202_Shake256_IncSqueeze(buf, SPX_DGST_BYTES, &s_inc);

  SLH_DSA_CommonLib_MemCpy(digest, bufp, SLH_DSA_SHAKE_192FSIMPLE_FORS_MSG_BYTES);
  bufp = &bufp[SLH_DSA_SHAKE_192FSIMPLE_FORS_MSG_BYTES];
...
} // end: SLH_DSA_SHAKE_192fSimple_HashMessage
