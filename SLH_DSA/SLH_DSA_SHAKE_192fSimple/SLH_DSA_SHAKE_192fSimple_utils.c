 ...
 ** @{ */
/** \addtogroup SLH_DSA_SHAKE_192fSimple_utils
 ** @{ */

/*====================================================================================================================*/
/** \file SLH_DSA_SHAKE_192fSimple_utils.c
* \brief  description of SLH_DSA_SHAKE_192fSimple_utils.c
*
* \details
*
*
 ...
 *
 */
void SLH_DSA_SHAKE_192fSimple_ComputeRoot(uint8 *const root, const uint8 *const leaf, uint32 leaf_idx,
                                               uint32 idx_offset, const uint8 *const auth_path, uint32 tree_height,
                                               const slh_dsa_shake_192f_ctx *const ctx, uint32 addr[8])
{
  uint32 i                                           = 0;
  uint8 buffer[2u * SLH_DSA_SHAKE_192FSIMPLE_N] = {0};
  ...
  {
    SLH_DSA_CommonLib_MemCpy(&buffer[SLH_DSA_SHAKE_192FSIMPLE_N], leaf, SLH_DSA_SHAKE_192FSIMPLE_N);
    SLH_DSA_CommonLib_MemCpy(buffer, auth_path_temp, SLH_DSA_SHAKE_192FSIMPLE_N);
  }
  else
  {
    SLH_DSA_CommonLib_MemCpy(buffer, leaf, SLH_DSA_SHAKE_192FSIMPLE_N);
    SLH_DSA_CommonLib_MemCpy(&buffer[SLH_DSA_SHAKE_192FSIMPLE_N], auth_path_temp, SLH_DSA_SHAKE_192FSIMPLE_N);
  }
  auth_path_temp = &auth_path_temp[SLH_DSA_SHAKE_192FSIMPLE_N];
  ...
  for (i = 0; i < (tree_height - 1u); i++)
  {
    ...
    if (0u < (leaf_idx_temp & 1u))
    {
      SLH_DSA_SHAKE_192fSimple_Thash(&buffer[SLH_DSA_SHAKE_192FSIMPLE_N], buffer, 2u, ctx, addr);
      SLH_DSA_CommonLib_MemCpy(buffer, auth_path_temp, SLH_DSA_SHAKE_192FSIMPLE_N);
    }
    else
    {
      SLH_DSA_SHAKE_192fSimple_Thash(buffer, buffer, 2u, ctx, addr);
      SLH_DSA_CommonLib_MemCpy(&buffer[SLH_DSA_SHAKE_192FSIMPLE_N], auth_path_temp, SLH_DSA_SHAKE_192FSIMPLE_N);
    }
    auth_path_temp = &auth_path_temp[SLH_DSA_SHAKE_192FSIMPLE_N];
  }
  ...
  SLH_DSA_SHAKE_SetTreeIndex(addr, leaf_idx_temp + idx_offset_temp);
  SLH_DSA_SHAKE_192fSimple_Thash(root, buffer, 2u, ctx, addr);
} // end: SLH_DSA_SHAKE_192fSimple_ComputeRoot
 ...
 *
 */
void SLH_DSA_SHAKE_192fSimple_TreeHash(uint8 *const root, uint8 *const auth_path,
                                            const slh_dsa_shake_192f_ctx *const ctx, uint32 leaf_idx, uint32 idx_offset,
                                            uint32 tree_height,
                                            void (*const gen_leaf)(uint8 *leaf, const slh_dsa_shake_192f_ctx *ctx,
                                                                   uint32 addr_idx, const uint32 tree_addr[8]),
                                            uint32 tree_addr[8])
{
  uint8 stack[(SLH_DSA_SHAKE_192FSIMPLE_TREEHASH_BUF_LEN + 1u) * SLH_DSA_SHAKE_192FSIMPLE_N] = {0};
  uint32 heights[SLH_DSA_SHAKE_192FSIMPLE_TREEHASH_BUF_LEN + 1u]                                  = {0};
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    gen_leaf(&stack[offset * SLH_DSA_SHAKE_192FSIMPLE_N], ctx, idx + idx_offset, tree_addr);
    offset++;
    heights[offset - 1u] = 0;
    ...
    {
      SLH_DSA_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * SLH_DSA_SHAKE_192FSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      SLH_DSA_SHAKE_192fSimple_Thash(&stack[(offset - 2u) * SLH_DSA_SHAKE_192FSIMPLE_N],
                                          &stack[(offset - 2u) * SLH_DSA_SHAKE_192FSIMPLE_N], 2u, ctx, tree_addr);
      offset--;
      ...
      {
        SLH_DSA_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * SLH_DSA_SHAKE_192FSIMPLE_N],
                               &stack[(offset - 1u) * SLH_DSA_SHAKE_192FSIMPLE_N],
                               SLH_DSA_SHAKE_192FSIMPLE_N);
      }
    }
  }
  SLH_DSA_CommonLib_MemCpy(root, stack, SLH_DSA_SHAKE_192FSIMPLE_N);
} // end: SLH_DSA_SHAKE_192fSimple_TreeHash
