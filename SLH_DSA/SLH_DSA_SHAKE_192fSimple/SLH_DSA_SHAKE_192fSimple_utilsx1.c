  ...
  for (idx = 0; idx < SLH_DSA_UINT32_MAX_VALUE; idx++)
  {
    ...
    for (h = 0; h < SLH_DSA_UINT32_MAX_VALUE; h++)
    {
      ...
      if (h == tree_height)
      {
        ...
        SLH_DSA_CommonLib_MemCpy(root, &current[SLH_DSA_SHAKE_192FSIMPLE_N], SLH_DSA_SHAKE_192FSIMPLE_N);
        bStopFunc = TRUE;
      }
      ...
      {
        SLH_DSA_CommonLib_MemCpy(&auth_path[h * SLH_DSA_SHAKE_192FSIMPLE_N],
      ...
      SLH_DSA_CommonLib_MemCpy(&current[0], left, SLH_DSA_SHAKE_192FSIMPLE_N);
    ...
    if (FALSE == bStopFunc)
    {
      ...
      SLH_DSA_CommonLib_MemCpy(&stack[h * SLH_DSA_SHAKE_192FSIMPLE_N], &current[SLH_DSA_SHAKE_192FSIMPLE_N],
