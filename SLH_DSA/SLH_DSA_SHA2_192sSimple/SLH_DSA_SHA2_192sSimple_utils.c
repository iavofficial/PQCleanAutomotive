                                              ...
                                              const slh_dsa_sha2_192s_ctx *const ctx, uint32 addr[8])
{
  ...
  {
    SLH_DSA_CommonLib_MemCpy(&buffer[SLH_DSA_SHA2_192SSIMPLE_N], leaf, SLH_DSA_SHA2_192SSIMPLE_N);
    SLH_DSA_CommonLib_MemCpy(buffer, auth_path_temp, SLH_DSA_SHA2_192SSIMPLE_N);
  }
  else
  {
    SLH_DSA_CommonLib_MemCpy(buffer, leaf, SLH_DSA_SHA2_192SSIMPLE_N);
    SLH_DSA_CommonLib_MemCpy(&buffer[SLH_DSA_SHA2_192SSIMPLE_N], auth_path_temp, SLH_DSA_SHA2_192SSIMPLE_N);
  }
                                           ...
                                           uint32 tree_addr[8])
{
  ...
  for (idx = 0; idx < (uint32)((uint32)1u << tree_height); idx++)
  {
    ...
    {
      SLH_DSA_CommonLib_MemCpy(auth_path, &stack[(offset - 1u) * SLH_DSA_SHA2_192SSIMPLE_N],
    ...
    while ((offset >= 2u) && (heights[offset - 1u] == heights[offset - 2u]))
    {
      ...
      {
        SLH_DSA_CommonLib_MemCpy(&auth_path[heights[offset - 1u] * SLH_DSA_SHA2_192SSIMPLE_N],
                               &stack[(offset - 1u) * SLH_DSA_SHA2_192SSIMPLE_N], SLH_DSA_SHA2_192SSIMPLE_N);
      }
    }
  }
  SLH_DSA_CommonLib_MemCpy(root, stack, SLH_DSA_SHA2_192SSIMPLE_N);
