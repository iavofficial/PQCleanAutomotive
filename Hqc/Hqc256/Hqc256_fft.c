 ...
 *
 * This function is a subroutine of the function Hqc256_fft.
 *
 * \param[out] w Array
 * \param[in] f Array
                        ...
                        const uint16 *const betas)
{
  ...
  if (m_f == 1)
  {
    for (uint8 y = 0; y < m; ++y)
    {
      tmp[y] = Hqc256_Gf_Mul(betas[y], f[1]);
    }
  ...
  else
  {
    ...
    {
      gammas[y] = Hqc256_Gf_Mul(betas[y], Hqc256_Gf_Inverse(betas[m - 1]));
      deltas[y] = Hqc256_Gf_Square(gammas[y]) ^ gammas[y];
    }
...
} // end: Hqc256_Fft_Retrieve_Error_Poly
