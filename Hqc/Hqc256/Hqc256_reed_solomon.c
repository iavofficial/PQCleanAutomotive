 ...
 * The last two elements are needed by the Hqc256_gf_mul function
...
static void compute_syndromes_256(uint16 *const syndromes, const uint8 *const cdw)
{
  for (uint8 i = 0; i < (2 * HQC256_PARAM_DELTA); ++i)
  {
    ...
    {
      syndromes[i] ^= Hqc256_Gf_Mul(cdw[j], alpha_ij_pow_256[i][j - 1]);
    }
    syndromes[i] ^= cdw[0];
  }
...
static uint16 hqc256_compute_elp(uint16 *const sigma, const uint16 *const syndromes)
{
  ...
  for (mu = 0; (mu < (2 * HQC256_PARAM_DELTA)); ++mu)
  {
    ...
    Hqc_CommonLib_MemCpy(sigma_copy, sigma, 2 * (HQC256_PARAM_DELTA));
...
*
* See function Hqc256_fft for more details.
*
...
static void compute_roots_256(uint8 *const err, const uint16 *const sigma)
{
  ...
  Hqc256_Fft_Retrieve_Error_Poly(err, w);
} // end: compute_roots
...
static void compute_error_values_256(uint16 *const error_values, const uint16 *const z, const uint8 *const err)
{
  ...
  for (uint8 i = 0; i < HQC256_PARAM_DELTA; ++i)
  {
    ...
    {
      tmp2 = Hqc256_Gf_Mul(tmp2, (1U ^ Hqc256_Gf_Mul(inverse, beta_j[(i + k) % HQC256_PARAM_DELTA])));
    }
...
void Hqc256_Reed_Solomon_Encode(uint8 *const cdw, const uint8 *const msg)
{
  ...
  Hqc_CommonLib_MemSet(cdw, 0, HQC256_PARAM_N1);
  ...
  for (uint8 i = 0; i < HQC256_PARAM_K; ++i)
  {
    ...
    {
      tmp[j] = Hqc256_Gf_Mul(gate_value, PARAM_RS_POLY[j]);
    }
  ...
  Hqc_CommonLib_MemCpy(&cdw[HQC256_PARAM_N1 - HQC256_PARAM_K], msg, HQC256_PARAM_K);
} // end: Hqc256_Reed_Solomon_Encode
...
void Hqc256_Reed_Solomon_Decode(uint8 *const msg, uint8 *const cdw)
{
  ...
  Hqc_CommonLib_MemCpy(msg, &cdw[HQC256_PARAM_G - 1], HQC256_PARAM_K);
} // end: Hqc256_Reed_Solomon_Decode
