...
sint8 Hqc192_Crypto_Kem_KeyPair(uint8 *const pk, uint8 *const sk)
{
  ...
  Hqc192_Pke_Keygen(pk, sk);
  return 0;
} // end: Hqc192_Crypto_Kem_KeyPair
...
sint8 Hqc192_Crypto_Kem_Enc(uint8 *const ct, uint8 *const ss, const uint8 *const pk)
{
  ...
  // Computing m
  (void)Hqc_CommonLib_RandomBytes(m, HQC192_VEC_K_SIZE_BYTES);

  // Computing theta
  (void)Hqc_CommonLib_RandomBytes(salt, HQC192_SALT_SIZE_BYTES);
  Hqc_CommonLib_MemCpy(&tmp[HQC192_VEC_K_SIZE_BYTES], pk, HQC192_PUBLIC_KEY_BYTES);
  Hqc192_Shake256_512_Ds(&shake256state, theta, tmp,
                               HQC192_VEC_K_SIZE_BYTES + HQC192_PUBLIC_KEY_BYTES + HQC192_SALT_SIZE_BYTES,
                               HQC192_G_FCT_DOMAIN);

  // Encrypting m
  Hqc192_Pke_Encrypt(u, v, m, theta, pk);

  // Computing shared secret
  Hqc_CommonLib_MemCpy(mc, m, HQC192_VEC_K_SIZE_BYTES);
  Hqc192_Store8_Arr(&mc[HQC192_VEC_K_SIZE_BYTES], HQC192_VEC_N_SIZE_BYTES, u, HQC192_VEC_N_SIZE_64);
  Hqc192_Store8_Arr(&mc[HQC192_VEC_K_SIZE_BYTES + HQC192_VEC_N_SIZE_BYTES], HQC192_VEC_N1N2_SIZE_BYTES, v,
                          HQC192_VEC_N1N2_SIZE_64);
  Hqc192_Shake256_512_Ds(&shake256state, ss, mc,
                               HQC192_VEC_K_SIZE_BYTES + HQC192_VEC_N_SIZE_BYTES + HQC192_VEC_N1N2_SIZE_BYTES,
                               HQC192_K_FCT_DOMAIN);

  // Computing ciphertext
  Hqc192_Ciphertext_To_String(ct, u, v, salt);

  return 0;
} // end: Hqc192_Crypto_Kem_Enc
...
sint8 Hqc192_Crypto_Kem_Dec(uint8 *const ss, const uint8 *const ct, const uint8 *const sk)
{
  ...
  Hqc192_Ciphertext_From_String(u, v, salt, ct);

  // Decrypting
  result = Hqc192_Pke_Decrypt(m, sigma, u, v, sk);
  ...
  // Encrypting m'
  Hqc192_Pke_Encrypt(u2, v2, m, theta, pk);

  // Check if c != c'
  result |= Hqc192_Vect_Compare((uint8 *)u, (uint8 *)u2, HQC192_VEC_N_SIZE_BYTES);
  result |= Hqc192_Vect_Compare((uint8 *)v, (uint8 *)v2, HQC192_VEC_N1N2_SIZE_BYTES);
